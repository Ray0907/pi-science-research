...
function validateSignal(value:AbortSignal|undefined):AbortSignal { const signal=value===undefined?new AbortController().signal:value;if(signal===null||typeof signal!=="object"||utilTypes.isProxy(signal))fail("network.invalid-input");nativeSignalAborted(signal);try{NATIVE_THROW_IF_ABORTED.call(signal);}catch{return fail("network.cancelled");}return signal; }

...
import { CanonicalJsonError, canonicalJson } from "../crypto/canonical-json.js";
...
const NATIVE_DOM_EXCEPTION_NAME_GETTER = Object.getOwnPropertyDescriptor(DOMException.prototype,"name")!.get!;
const MAX_AGGREGATE_RESOLVER_ERRORS = 2;
...
function hasUnpairedSurrogate(value:string):boolean { for(let index=0;index<value.length;index+=1){const unit=value.charCodeAt(index);if(unit>=0xd800&&unit<=0xdbff){if(index+1>=value.length)return true;const next=value.charCodeAt(index+1);if(next<0xdc00||next>0xdfff)return true;index+=1;}else if(unit>=0xdc00&&unit<=0xdfff)return true;}return false; }
function mappedCanonicalJson(value:unknown,code:NetworkPolicyErrorCode):string { try{return canonicalJson(value);}catch(error){if(error instanceof CanonicalJsonError)return fail(code);throw error;} }
function canonicalStringBytes(value:string,code:NetworkPolicyErrorCode):number { if(hasUnpairedSurrogate(value))fail(code);if(value.length>16_384)return 16_385;return Buffer.byteLength(mappedCanonicalJson(value,code),"utf8"); }
...
export function validateFixedProviderUrl(target:AcquisitionNetworkTarget,input:unknown):ValidatedProviderUrl {
  if(target!=="crossref"&&target!=="openalex"&&target!=="ncbi")fail("network.invalid-input");if(typeof input!=="string"||hasUnpairedSurrogate(input))fail("network.invalid-input");if(input.length>16_384||canonicalStringBytes(input,"network.invalid-input")>16_384)fail("network.url-too-large");
...
function nativeSignalAborted(value:object):boolean { try{const aborted=NATIVE_ABORTED_GETTER.call(value) as unknown;if(typeof aborted!=="boolean")fail("network.invalid-input");return aborted;}catch(error){if(error instanceof NetworkPolicyError)throw error;return fail("network.invalid-input");} }
type AuthenticatedSignal = Readonly<{signal:AbortSignal;initiallyAborted:boolean}>;
function authenticateSignal(value:AbortSignal|undefined):AuthenticatedSignal { const signal=value===undefined?new AbortController().signal:value;if(signal===null||typeof signal!=="object"||utilTypes.isProxy(signal))fail("network.invalid-input");const initiallyAborted=nativeSignalAborted(signal);let throwIfAbortedThrew=false;try{NATIVE_THROW_IF_ABORTED.call(signal);}catch{throwIfAbortedThrew=true;}if(throwIfAbortedThrew!==initiallyAborted)fail("network.invalid-input");return {signal,initiallyAborted}; }
function validateProviderUrlSnapshot(value:ValidatedProviderUrl,maximum:number):ValidatedProviderUrl {
  const descriptors=dataDescriptors(value,["target","url","origin","hostname","port","hostHeader"],"network.invalid-input");for(const key of ["target","url","origin","hostname","port","hostHeader"])if(!descriptors[key])fail("network.invalid-input");const url=descriptors.url!.value;if(typeof url!=="string"||hasUnpairedSurrogate(url))fail("network.invalid-input");if(url.length>maximum||Buffer.byteLength(mappedCanonicalJson(url,"network.invalid-input"),"utf8")>maximum)fail("network.url-too-large");const rebuilt=validateFixedProviderUrl(descriptors.target!.value as AcquisitionNetworkTarget,url);for(const key of ["target","url","origin","hostname","port","hostHeader"] as const)if(descriptors[key]!.value!==rebuilt[key])fail("network.invalid-input");return rebuilt;
}
...
function nativeDomExceptionName(value:unknown):string|undefined { if(value===null||typeof value!=="object"||utilTypes.isProxy(value))return undefined;try{return NATIVE_DOM_EXCEPTION_NAME_GETTER.call(value) as string;}catch{return undefined;} }
function aggregateResolverErrors(value:unknown):readonly unknown[]|undefined { if(value===null||typeof value!=="object"||utilTypes.isProxy(value)||!utilTypes.isNativeError(value))return undefined;let prototype:object|null;try{prototype=Object.getPrototypeOf(value);}catch{return undefined;}if(prototype!==AggregateError.prototype)return undefined;let descriptor:PropertyDescriptor|undefined;try{descriptor=Object.getOwnPropertyDescriptor(value,"errors");}catch{return undefined;}if(!descriptor||!("value" in descriptor))return undefined;const errors=descriptor.value;if(errors===null||typeof errors!=="object"||utilTypes.isProxy(errors)||!Array.isArray(errors))return undefined;let lengthDescriptor:PropertyDescriptor|undefined;try{lengthDescriptor=Object.getOwnPropertyDescriptor(errors,"length");}catch{return undefined;}if(!lengthDescriptor||!("value" in lengthDescriptor)||typeof lengthDescriptor.value!=="number"||!Number.isSafeInteger(lengthDescriptor.value)||lengthDescriptor.value<1||lengthDescriptor.value>MAX_AGGREGATE_RESOLVER_ERRORS||typeof lengthDescriptor.writable!=="boolean"||lengthDescriptor.enumerable!==false||lengthDescriptor.configurable!==false)return undefined;let prototypeOfErrors:object|null;try{prototypeOfErrors=Object.getPrototypeOf(errors);}catch{return undefined;}if(prototypeOfErrors!==Array.prototype)return undefined;const length=lengthDescriptor.value;let descriptors:PropertyDescriptorMap;try{descriptors=Object.getOwnPropertyDescriptors(errors as object) as PropertyDescriptorMap;}catch{return undefined;}const keys=Reflect.ownKeys(descriptors);if(keys.some((key)=>typeof key!=="string")||keys.length!==length+1)return undefined;const output:unknown[]=[];for(let index=0;index<length;index+=1){const item=descriptors[String(index)];if(!item||!("value" in item)||!item.enumerable)return undefined;output.push(item.value);}return output; }
type ResolverErrorClass="abort"|"empty"|"nxdomain"|"temporary";
function classifyResolverError(value:unknown):ResolverErrorClass { if(value===null||typeof value!=="object"||utilTypes.isProxy(value))return"temporary";if(nativeDomExceptionName(value)==="AbortError")return"abort";const code=ownErrorCode(value);if(code==="ABORT_ERR")return"abort";if(code==="ENODATA"||code==="EAI_NODATA")return"empty";if(code==="ENOTFOUND"||code==="EAI_NONAME")return"nxdomain";return"temporary"; }
function mapResolverError(error:unknown):never { let aggregate=false;if(error!==null&&typeof error==="object"&&!utilTypes.isProxy(error)&&utilTypes.isNativeError(error)){try{aggregate=Object.getPrototypeOf(error)===AggregateError.prototype;}catch{aggregate=false;}}if(aggregate){const nested=aggregateResolverErrors(error);if(!nested)fail("network.dns-temporary");const classes=nested.map(classifyResolverError);if(classes.includes("nxdomain"))fail("network.dns-nxdomain");if(classes.includes("temporary"))fail("network.dns-temporary");if(classes.every((value)=>value==="empty"))fail("network.dns-empty");if(classes.includes("abort"))fail("network.cancelled");return fail("network.dns-temporary");}const classification=classifyResolverError(error);if(classification==="empty")fail("network.dns-empty");if(classification==="nxdomain")fail("network.dns-nxdomain");if(classification==="abort")fail("network.cancelled");return fail("network.dns-temporary"); }
...
export async function resolveAllSafeProviderAddressesInternal(validated:ValidatedProviderUrl,options:NetworkPolicyOptions|undefined,resolver:AcquisitionDnsResolver,signal?:AbortSignal):Promise<SafeProviderAddressSetInternal> {
  const normalizedOptions=normalizeOptions(options);const state=resolverState(resolver);const authenticatedSignal=authenticateSignal(signal);const providerUrl=validateProviderUrlSnapshot(validated,normalizedOptions.maxCanonicalUrlBytes);const ownedSignal=authenticatedSignal.signal;if(authenticatedSignal.initiallyAborted||nativeSignalAborted(ownedSignal))fail("network.cancelled");let raw:unknown;try{raw=await state.resolveAll(providerUrl.hostname,ownedSignal);}catch(error){return mapResolverError(error);}const values=rawAnswerValues(raw,normalizedOptions);if(values.length===0)fail("network.dns-empty");const scalarBudget={bytes:0};const parsed=values.map((value)=>parseDnsAnswer(value,normalizedOptions,scalarBudget));const hasPublic=parsed.some((item)=>item.classification==="public");const hasForbidden=parsed.some((item)=>item.classification==="forbidden");if(hasPublic&&hasForbidden)fail("network.dns-mixed-addresses");if(hasForbidden)fail("network.dns-address-forbidden");if(nativeSignalAborted(ownedSignal))fail("network.cancelled");const unique=new Map<string,ParsedAddress>();for(const item of parsed){const key=`${item.family}:${Buffer.from(item.bytes).toString("hex")}`;if(!unique.has(key))unique.set(key,item);}const ordered=[...unique.values()].sort(compareParsed);const addresses=Object.freeze(ordered.map((item)=>Object.freeze({address:item.address,family:item.family})));const allAddressesSha256=sha256Hex(mappedCanonicalJson({schemaVersion:1,addresses},"network.dns-invalid-address"));return Object.freeze({addresses,allAddressesSha256});
}

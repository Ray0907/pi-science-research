...
function assertBoundedStructure(value:unknown,limits:StructuralLimits):void {
  const active=new WeakSet<object>();const descriptorsByObject=new WeakMap<object,PropertyDescriptorMap>();let nodes=0;let keys=0;
  const visitShape=(item:unknown,depth:number):void=>{nodes+=1;if(nodes>limits.maxNodes||depth>limits.maxDepth)throw new StructuralLimitError("limit");if(typeof item==="string")return;if(item===null||typeof item==="boolean")return;if(typeof item==="number"){if(!Number.isFinite(item))throw new StructuralLimitError("unsafe");return;}if(typeof item!=="object"||utilTypes.isProxy(item))throw new StructuralLimitError("unsafe");if(active.has(item))throw new StructuralLimitError("unsafe");const array=Array.isArray(item);const prototype=Object.getPrototypeOf(item);if((array&&prototype!==Array.prototype)||(!array&&prototype!==Object.prototype&&prototype!==null))throw new StructuralLimitError("unsafe");const descriptors=Object.getOwnPropertyDescriptors(item);descriptorsByObject.set(item,descriptors);const ownKeys=Reflect.ownKeys(descriptors);if(ownKeys.some((key)=>typeof key!=="string"))throw new StructuralLimitError("unsafe");if(array&&(item as unknown[]).length>limits.maxArrayLength)throw new StructuralLimitError("limit");active.add(item);try{for(const key of ownKeys as string[]){if(array&&key==="length")continue;keys+=1;if(keys>limits.maxKeys)throw new StructuralLimitError("limit");const descriptor=descriptors[key]!;if(!("value"in descriptor)||!descriptor.enumerable)throw new StructuralLimitError("unsafe");if(array&&(!/^(0|[1-9][0-9]*)$/u.test(key)||Number(key)>=(item as unknown[]).length))throw new StructuralLimitError("unsafe");visitShape(descriptor.value,depth+1);}if(array&&ownKeys.length!==(item as unknown[]).length+1)throw new StructuralLimitError("unsafe");}finally{active.delete(item);}};
  visitShape(value,0);
  let canonicalScalarBytes=0;const addScalar=(scalar:unknown):void=>{const bytes=Buffer.byteLength(canonicalJson(scalar),"utf8");if(typeof scalar==="string"&&bytes>limits.maxStringBytes)throw new StructuralLimitError("limit");canonicalScalarBytes+=bytes;if(!Number.isSafeInteger(canonicalScalarBytes)||canonicalScalarBytes>limits.maxScalarBytes)throw new StructuralLimitError("limit");};const checkKey=(key:string):void=>{if(Buffer.byteLength(canonicalJson(key),"utf8")>limits.maxStringBytes)throw new StructuralLimitError("limit");};
  const visitCanonical=(item:unknown):void=>{if(typeof item==="string"||item===null||typeof item==="boolean"||typeof item==="number"){addScalar(item);return;}const descriptors=descriptorsByObject.get(item as object);if(!descriptors)throw new StructuralLimitError("unsafe");const array=Array.isArray(item);for(const key of Reflect.ownKeys(descriptors) as string[]){if(array&&key==="length")continue;checkKey(key);visitCanonical(descriptors[key]!.value);}};
  visitCanonical(value);
}
function assertSafeStructure(value:unknown,limits:StructuralLimits):void { const seen=new WeakSet<object>();let nodes=0;let keys=0;const visit=(item:unknown,depth:number):void=>{nodes+=1;if(nodes>limits.maxNodes||depth>limits.maxDepth)throw new StructuralLimitError("limit");if(item===null||typeof item==="string"||typeof item==="boolean")return;if(typeof item==="number"){if(!Number.isFinite(item))throw new StructuralLimitError("unsafe");return;}if(typeof item!=="object"||utilTypes.isProxy(item)||seen.has(item))throw new StructuralLimitError("unsafe");const array=Array.isArray(item);const prototype=Object.getPrototypeOf(item);if((array&&prototype!==Array.prototype)||(!array&&prototype!==Object.prototype&&prototype!==null))throw new StructuralLimitError("unsafe");const descriptors=Object.getOwnPropertyDescriptors(item);const ownKeys=Reflect.ownKeys(descriptors);if(ownKeys.some((key)=>typeof key!=="string"))throw new StructuralLimitError("unsafe");if(array&&(item as unknown[]).length>limits.maxArrayLength)throw new StructuralLimitError("limit");seen.add(item);for(const key of ownKeys as string[]){if(array&&key==="length")continue;keys+=1;if(keys>limits.maxKeys)throw new StructuralLimitError("limit");const descriptor=descriptors[key]!;if(!("value" in descriptor)||!descriptor.enumerable)throw new StructuralLimitError("unsafe");if(array&&(!/^(0|[1-9][0-9]*)$/u.test(key)||Number(key)>=(item as unknown[]).length))throw new StructuralLimitError("unsafe");visit(descriptor.value,depth+1);}if(array&&ownKeys.length!==(item as unknown[]).length+1)throw new StructuralLimitError("unsafe");};visit(value,0);}
function preflightSafeStructure(value:unknown,limits:StructuralLimits,invalid:AcquisitionContractErrorCode,tooLarge:AcquisitionContractErrorCode):void { try{assertSafeStructure(value,limits);}catch(error){if(error instanceof StructuralLimitError&&error.reason==="limit")fail(tooLarge);fail(invalid);} }
...
/** Opaque observation-sink type. Task 9 owns its executable descriptor and factory. */
...
type IndexedPartition = { snapshot:ProviderRequestPartitionSnapshotInternal;canonicalSnapshot:string;liveRegistrations:number };
...
const partitionIndexByKey = new Map<string,IndexedPartition>();
const partitionIndexByHash = new Map<string,IndexedPartition>();
...
function descriptorRecord(value:unknown,allowed:readonly string[],code:AcquisitionContractErrorCode):PropertyDescriptorMap {
  if(!plain(value))fail(code);const descriptors=Object.getOwnPropertyDescriptors(value);const keys=Reflect.ownKeys(descriptors);if(keys.some((key)=>typeof key!=="string"||!allowed.includes(key)))fail(code);for(const key of keys as string[]){const descriptor=descriptors[key]!;if(!descriptor.enumerable||!("value" in descriptor))fail(code);}return descriptors;
}
function preflightArray(value:unknown,maximum:number,invalid:AcquisitionContractErrorCode,tooLarge:AcquisitionContractErrorCode):void {
  if(value===null||typeof value!=="object"||utilTypes.isProxy(value))fail(invalid);let array:boolean;try{array=Array.isArray(value)&&Object.getPrototypeOf(value)===Array.prototype;}catch{return fail(invalid);}if(!array)fail(invalid);const length=Object.getOwnPropertyDescriptor(value,"length");if(!length||!("value" in length)||typeof length.value!=="number"||!Number.isSafeInteger(length.value)||length.value<0)fail(invalid);if(length.value>maximum)fail(tooLarge);
}
...
function snapshot<T>(value:unknown,limits:StructuralLimits,invalid:AcquisitionContractErrorCode,tooLarge:AcquisitionContractErrorCode):T {
  try { assertBoundedStructure(value,limits);rejectAliases(value);return JSON.parse(canonicalJson(value)) as T; }
...
export function normalizeAcquisitionOptions(raw:unknown):NormalizedAcquisitionOptionsInternal {
  let data:Record<string,unknown>={};const allowed=["transport","providers",...topKeys];
  if(raw!==undefined){preflightSafeStructure(raw,OPTION_STRUCTURE,"acquisition-contract.invalid-options","acquisition-contract.invalid-options");const descriptors=descriptorRecord(raw,allowed,"acquisition-contract.invalid-options");if(descriptors.transport)descriptorRecord(descriptors.transport.value,transportKeys,"acquisition-contract.invalid-options");if(descriptors.providers)descriptorRecord(descriptors.providers.value,providerKeys,"acquisition-contract.invalid-options");try{assertBoundedStructure(raw,OPTION_STRUCTURE);data=JSON.parse(canonicalJson(raw)) as Record<string,unknown>;}catch{fail("acquisition-contract.invalid-options");}}
...
function preflightNormalizedInputShape(value:unknown,code:AcquisitionContractErrorCode):void { const base=descriptorRecord(value,["kind","query","limit","doi","pmids","pmid","pmcid"],code);const kind=base.kind?.value;const allowed=kind==="query"?["kind","query","limit"]:kind==="doi"?["kind","doi"]:kind==="pmid-list"?["kind","pmids"]:kind==="pmid-link"?["kind","pmid"]:kind==="pmcid"?["kind","pmcid"]:[];if(allowed.length===0)fail(code);descriptorRecord(value,allowed,code); }
...
function fixedUrl(value:unknown,target:"crossref"|"openalex"|"ncbi",maximum=16_384):string {
  if(typeof value!=="string")fail("acquisition-contract.invalid-input");let url:URL;try{url=new URL(value);}catch{return fail("acquisition-contract.invalid-input");}
...
export function createProviderPartitionKey(input:Readonly<{provider:AcquisitionProvider;operation:AcquisitionOperation;endpointClass:string;target:"crossref"|"openalex"|"ncbi";normalizedInput:ProviderNormalizedInput;requestedUrl:string;}>):Readonly<{partitionKey:`partition-v1-${string}`;partitionKeySha256:string}> {
  preflightSafeStructure(input,PUBLIC_STRUCTURE,"acquisition-contract.invalid-input","acquisition-contract.input-too-large");const inputDescriptors=descriptorRecord(input,["provider","operation","endpointClass","target","normalizedInput","requestedUrl"],"acquisition-contract.invalid-input");if(!inputDescriptors.normalizedInput)fail("acquisition-contract.invalid-input");preflightNormalizedInputShape(inputDescriptors.normalizedInput.value,"acquisition-contract.invalid-input");const data=snapshot<Record<string,unknown>>(input,PUBLIC_STRUCTURE,"acquisition-contract.invalid-input","acquisition-contract.input-too-large");const v=requireClosed(data,["provider","operation","endpointClass","target","normalizedInput","requestedUrl"],"acquisition-contract.invalid-input");
...
export function registerProviderRequestPartitionInternal(owner:ProviderPartitionPlanOwnerInternal,input:unknown):ProviderRequestPartitionInternal { const state=ownerState(owner);const limits:StructuralLimits={maxDepth:state.options.maxStructureDepth,maxNodes:state.options.maxStructureNodes,maxKeys:state.options.maxStructureKeys,maxArrayLength:state.options.maxStructureNodes,maxStringBytes:state.options.maxStringCanonicalBytes,maxScalarBytes:state.options.maxScalarCanonicalBytes};preflightSafeStructure(input,limits,"acquisition-contract.invalid-input","acquisition-contract.input-too-large");const inputDescriptors=descriptorRecord(input,["provider","operation","endpointClass","partitionKey","partitionKeySha256","normalizedInput","normalizedInputSha256","target","url"],"acquisition-contract.invalid-input");if(!inputDescriptors.normalizedInput)fail("acquisition-contract.invalid-input");preflightNormalizedInputShape(inputDescriptors.normalizedInput.value,"acquisition-contract.invalid-input");if(state.count>=state.options.maxPartitions)fail("acquisition-contract.input-too-large");const data=snapshot<Record<string,unknown>>(input,limits,"acquisition-contract.invalid-input","acquisition-contract.input-too-large");const keys=["provider","operation","endpointClass","partitionKey","partitionKeySha256","normalizedInput","normalizedInputSha256","target","url"];const v=requireClosed(data,keys,"acquisition-contract.invalid-input");for(const required of keys)if(!Object.hasOwn(v,required))fail("acquisition-contract.invalid-input");
  ...
  const snapshotValue=deepFreeze({provider,operation,endpointClass,partitionKey:expected.partitionKey,partitionKeySha256:expected.partitionKeySha256,normalizedInput,normalizedInputSha256:v.normalizedInputSha256 as string,target,url}) as ProviderRequestPartitionSnapshotInternal;const canonicalSnapshot=canonicalJson(snapshotValue);const keyed=partitionIndexByKey.get(expected.partitionKey);const hashed=partitionIndexByHash.get(expected.partitionKeySha256);if((keyed!==undefined&&keyed.canonicalSnapshot!==canonicalSnapshot)||(hashed!==undefined&&hashed.canonicalSnapshot!==canonicalSnapshot)||(keyed!==undefined&&hashed!==undefined&&keyed!==hashed))fail("acquisition-contract.invalid-key");const indexed=keyed??hashed??{snapshot:snapshotValue,canonicalSnapshot,liveRegistrations:0};if(keyed===undefined)partitionIndexByKey.set(expected.partitionKey,indexed);if(hashed===undefined)partitionIndexByHash.set(expected.partitionKeySha256,indexed);indexed.liveRegistrations=checkedAdd(indexed.liveRegistrations,1,"acquisition-contract.invalid-capability");const partition=Object.freeze({capabilityKind:"provider-request-partition" as const});partitionRegistry.set(partition,{owner,snapshot:snapshotValue});state.keys.add(expected.partitionKey);state.partitions.add(partition);state.count+=1;return partition;
}
...
export function closeProviderPartitionPlanOwnerInternal(owner:ProviderPartitionPlanOwnerInternal):void { const state=ownerRegistry.get(owner as object);if(!state)fail("acquisition-contract.invalid-capability");if(!state.open)return;state.open=false;for(const partition of state.partitions){const snapshot=partitionRegistry.get(partition)!.snapshot;const indexed=partitionIndexByKey.get(snapshot.partitionKey);if(!indexed||indexed!==partitionIndexByHash.get(snapshot.partitionKeySha256)||indexed.liveRegistrations<1)fail("acquisition-contract.invalid-capability");indexed.liveRegistrations-=1;}state.partitions.clear();state.keys.clear(); }
export function createProviderRequestPartitionFixtureInternal(input:Readonly<{partition:unknown;options:ProviderPartitionRegistryOptionsInternal;}>):ProviderRequestPartitionInternal { preflightSafeStructure(input,OPTION_STRUCTURE,"acquisition-contract.invalid-capability","acquisition-contract.invalid-capability");const descriptors=descriptorRecord(input,["partition","options"],"acquisition-contract.invalid-capability");if(!descriptors.partition||!descriptors.options)fail("acquisition-contract.invalid-capability");registryOptions(descriptors.options.value);const partitionDescriptors=descriptorRecord(descriptors.partition.value,["provider","operation","endpointClass","partitionKey","partitionKeySha256","normalizedInput","normalizedInputSha256","target","url"],"acquisition-contract.invalid-capability");if(!partitionDescriptors.normalizedInput)fail("acquisition-contract.invalid-capability");preflightNormalizedInputShape(partitionDescriptors.normalizedInput.value,"acquisition-contract.invalid-capability");let data:Record<string,unknown>;try{assertBoundedStructure(input,OPTION_STRUCTURE);data=JSON.parse(canonicalJson(input)) as Record<string,unknown>;}catch{return fail("acquisition-contract.invalid-capability");}const v=requireClosed(data,["partition","options"],"acquisition-contract.invalid-capability");const owner=createProviderPartitionPlanOwnerInternal({planId:`plan-v1-${sha256Hex(canonicalJson(v.partition))}`,options:v.options as ProviderPartitionRegistryOptionsInternal});return registerProviderRequestPartitionInternal(owner,v.partition); }

function validateProjectionLimits(value:unknown):AcquisitionTraceProjectionLimitsInternal { preflightSafeStructure(value,OPTION_STRUCTURE,"acquisition-contract.invalid-input","acquisition-contract.result-too-large");descriptorRecord(value,["maxCanonicalUrlBytes","maxRedirects"],"acquisition-contract.invalid-input");const data=snapshot<Record<string,unknown>>(value,OPTION_STRUCTURE,"acquisition-contract.invalid-input","acquisition-contract.result-too-large");const v=requireClosed(data,["maxCanonicalUrlBytes","maxRedirects"],"acquisition-contract.invalid-input");return {maxCanonicalUrlBytes:integer(v.maxCanonicalUrlBytes,16_384,"acquisition-contract.invalid-input"),maxRedirects:integer(v.maxRedirects,5,"acquisition-contract.invalid-input")}; }
...
function providerOrigins(provider:AcquisitionProvider):readonly string[] { return provider==="crossref"?["https://api.crossref.org"]:provider==="openalex"?["https://api.openalex.org"]:["https://eutils.ncbi.nlm.nih.gov","https://www.ncbi.nlm.nih.gov"]; }
function settlementCode(settlement:TransportSettlement):AcquisitionTraceSettlementCode { if(settlement.outcome==="success")return"success";switch(settlement.failureCode){case"network.dns-empty":case"network.dns-nxdomain":case"network.dns-temporary":case"transport.connect-failed":case"transport.connect-timeout":return"availability";case"transport.cancelled":return"cancelled";case"transport.deadline-exceeded":return"deadline-exceeded";case"transport.http-retryable":return"http-retryable";case"transport.http-terminal":return"http-terminal";case"transport.json-root-invalid":return"invalid-response";default:return"security-policy";} }
function preflightSettlementShape(value:unknown):PropertyDescriptorMap { const common=["schemaVersion","partitionKey","partitionKeySha256","provider","operation","endpointClass","normalizedOrigin","requestedUrl","finalUrl","redirectWitnesses","httpStatus","encodedBytes","decodedBytes","responsePayloadSha256","responseHeaders","responseHeadersSha256","connectedPeer","startedAt","settledAt","outcome","failureCode","payloadUtf8"];const descriptors=descriptorRecord(value,common,"acquisition-contract.invalid-input");for(const key of common)if(!descriptors[key])fail("acquisition-contract.invalid-input");preflightArray(descriptors.redirectWitnesses!.value,5,"acquisition-contract.invalid-input","acquisition-contract.result-too-large");preflightSafeStructure(value,PUBLIC_STRUCTURE,"acquisition-contract.invalid-input","acquisition-contract.input-too-large");for(const witness of descriptors.redirectWitnesses!.value as unknown[])descriptorRecord(witness,["ordinal","status","fromUrl","fromOrigin","toUrl","toOrigin"],"acquisition-contract.invalid-input");descriptorRecord(descriptors.responseHeaders!.value,["contentType","retryAfter","etag","lastModified"],"acquisition-contract.invalid-input");if(descriptors.connectedPeer!.value!==null)descriptorRecord(descriptors.connectedPeer!.value,["address","family"],"acquisition-contract.invalid-input");return descriptors; }
function resolveIndexedPartition(partitionKey:unknown,partitionKeySha256:unknown):ProviderRequestPartitionSnapshotInternal { const key=keyValue(partitionKey,KEY_PARTITION,"acquisition-contract.invalid-key");const hash=hashValue(partitionKeySha256,"acquisition-contract.invalid-key");if(key!==`partition-v1-${hash}`)fail("acquisition-contract.invalid-key");const keyed=partitionIndexByKey.get(key);const hashed=partitionIndexByHash.get(hash);if(!keyed||keyed!==hashed||keyed.liveRegistrations<1)fail("acquisition-contract.invalid-capability");return keyed.snapshot; }
const MAX_DECODED_BODY_BYTES = 67_108_864;
function validateSettlement(value:unknown,partition:ProviderRequestPartitionSnapshotInternal,limits:AcquisitionTraceProjectionLimitsInternal):TransportSettlement {
  const common=["schemaVersion","partitionKey","partitionKeySha256","provider","operation","endpointClass","normalizedOrigin","requestedUrl","finalUrl","redirectWitnesses","httpStatus","encodedBytes","decodedBytes","responsePayloadSha256","responseHeaders","responseHeadersSha256","connectedPeer","startedAt","settledAt","outcome","failureCode","payloadUtf8"];
  const descriptors=descriptorRecord(value,common,"acquisition-contract.invalid-input");for(const key of common)if(!Object.hasOwn(descriptors,key))fail("acquisition-contract.invalid-input");
  const redirectsRaw=descriptors.redirectWitnesses!.value;preflightArray(redirectsRaw,Math.min(5,limits.maxRedirects),"acquisition-contract.invalid-input","acquisition-contract.result-too-large");
  const payload=descriptors.payloadUtf8!.value;const outcome=descriptors.outcome!.value;const declaredDecodedBytes=descriptors.decodedBytes!.value;if(typeof declaredDecodedBytes==="number"&&Number.isSafeInteger(declaredDecodedBytes)&&declaredDecodedBytes>MAX_DECODED_BODY_BYTES)fail("acquisition-contract.result-too-large");
  if(outcome==="success"){
    if(typeof payload!=="string")fail("acquisition-contract.invalid-input");if(payload.length>MAX_DECODED_BODY_BYTES)fail("acquisition-contract.result-too-large");const payloadBytes=Buffer.byteLength(payload,"utf8");if(payloadBytes>MAX_DECODED_BODY_BYTES)fail("acquisition-contract.result-too-large");if(descriptors.decodedBytes!.value!==payloadBytes)fail("acquisition-contract.invalid-input");
  }else if(payload!==null)fail("acquisition-contract.invalid-input");
  const metadata:Record<string,unknown>={};for(const key of common)if(key!=="payloadUtf8")metadata[key]=descriptors[key]!.value;
  const v=snapshot<Record<string,unknown>>(metadata,PUBLIC_STRUCTURE,"acquisition-contract.invalid-input","acquisition-contract.input-too-large");
  if(v.schemaVersion!==1)fail("acquisition-contract.invalid-input");
  const partitionKey=keyValue(v.partitionKey,KEY_PARTITION,"acquisition-contract.invalid-key");const partitionKeySha256=hashValue(v.partitionKeySha256,"acquisition-contract.invalid-key");
  const provider=oneOf(v.provider,PROVIDERS,"acquisition-contract.invalid-input");const operation=oneOf(v.operation,OPERATIONS,"acquisition-contract.invalid-input");const endpointClass=stringValue(v.endpointClass,"acquisition-contract.invalid-input")!;
  if(partitionKey!==partition.partitionKey||partitionKeySha256!==partition.partitionKeySha256||provider!==partition.provider||operation!==partition.operation||endpointClass!==partition.endpointClass||v.requestedUrl!==partition.url)fail("acquisition-contract.invalid-key");
  const allowedOrigins=providerOrigins(provider);const requestedUrl=canonicalTransportUrl(v.requestedUrl,limits.maxCanonicalUrlBytes);const requestedOrigin=new URL(requestedUrl).origin;const normalizedOrigin=stringValue(v.normalizedOrigin,"acquisition-contract.invalid-input")!;
  if(normalizedOrigin!==requestedOrigin||!allowedOrigins.includes(requestedOrigin))fail("acquisition-contract.invalid-input");
  const finalUrl=v.finalUrl===null?null:canonicalTransportUrl(v.finalUrl,limits.maxCanonicalUrlBytes);if(finalUrl!==null&&!allowedOrigins.includes(new URL(finalUrl).origin))fail("acquisition-contract.invalid-input");
  const redirectWitnesses=v.redirectWitnesses as unknown[];let priorUrl=requestedUrl;let expectedOrdinal=1;
  for(const item of redirectWitnesses){const r=requireClosed(item,["ordinal","status","fromUrl","fromOrigin","toUrl","toOrigin"],"acquisition-contract.invalid-input");if(r.ordinal!==expectedOrdinal||![301,302,303,307,308].includes(r.status as number))fail("acquisition-contract.invalid-input");expectedOrdinal+=1;const fromUrl=canonicalTransportUrl(r.fromUrl,limits.maxCanonicalUrlBytes);const toUrl=canonicalTransportUrl(r.toUrl,limits.maxCanonicalUrlBytes);const fromOrigin=stringValue(r.fromOrigin,"acquisition-contract.invalid-input")!;const toOrigin=stringValue(r.toOrigin,"acquisition-contract.invalid-input")!;if(fromUrl!==priorUrl||new URL(fromUrl).origin!==fromOrigin||new URL(toUrl).origin!==toOrigin||!allowedOrigins.includes(fromOrigin)||!allowedOrigins.includes(toOrigin))fail("acquisition-contract.invalid-input");priorUrl=toUrl;}
  if(finalUrl===null){if(redirectWitnesses.length!==0)fail("acquisition-contract.invalid-input");}else if(finalUrl!==(redirectWitnesses.length===0?requestedUrl:priorUrl))fail("acquisition-contract.invalid-input");
  if(v.httpStatus!==null)integer(v.httpStatus,999,"acquisition-contract.invalid-input");nonnegative(v.encodedBytes,"acquisition-contract.invalid-input");const decodedBytes=nonnegative(v.decodedBytes,"acquisition-contract.invalid-input");if(decodedBytes>MAX_DECODED_BODY_BYTES)fail("acquisition-contract.result-too-large");if(v.responsePayloadSha256!==null)hashValue(v.responsePayloadSha256,"acquisition-contract.invalid-input");if(v.responseHeadersSha256!==null)hashValue(v.responseHeadersSha256,"acquisition-contract.invalid-input");
  const headers=requireClosed(v.responseHeaders,["contentType","retryAfter","etag","lastModified"],"acquisition-contract.invalid-input");for(const key of ["contentType","retryAfter","etag","lastModified"])stringValue(headers[key],"acquisition-contract.invalid-input",true);if(v.connectedPeer!==null){const peer=requireClosed(v.connectedPeer,["address","family"],"acquisition-contract.invalid-input");stringValue(peer.address,"acquisition-contract.invalid-input");if(peer.family!==4&&peer.family!==6)fail("acquisition-contract.invalid-input");}timestamp(v.startedAt,"acquisition-contract.invalid-input");timestamp(v.settledAt,"acquisition-contract.invalid-input");
  if(outcome==="success"){if(v.failureCode!==null||v.responsePayloadSha256===null||finalUrl===null)fail("acquisition-contract.invalid-input");}else if(outcome==="failure"){oneOf(v.failureCode,TRANSPORT_FAILURES,"acquisition-contract.invalid-input");if(v.failureCode==="transport.cancelled"||v.failureCode==="transport.deadline-exceeded")fail("acquisition-contract.invalid-input");}else if(outcome==="cancelled"){if(!["transport.cancelled","transport.deadline-exceeded"].includes(v.failureCode as string))fail("acquisition-contract.invalid-input");}else fail("acquisition-contract.invalid-input");
  return deepFreeze({...v,payloadUtf8:payload}) as unknown as TransportSettlement;
}
export function acquisitionTraceSettlementInternal(settlement:TransportSettlement,limits:AcquisitionTraceProjectionLimitsInternal):AcquisitionTraceSettlement { const descriptors=preflightSettlementShape(settlement);const normalizedLimits=validateProjectionLimits(limits);const partitionSnapshot=resolveIndexedPartition(descriptors.partitionKey?.value,descriptors.partitionKeySha256?.value);const value=validateSettlement(settlement,partitionSnapshot,normalizedLimits);const urlSlots=checkedMultiply(checkedAdd(2,checkedMultiply(2,value.redirectWitnesses.length,"acquisition-contract.result-too-large"),"acquisition-contract.result-too-large"),normalizedLimits.maxCanonicalUrlBytes,"acquisition-contract.result-too-large");if(!Number.isSafeInteger(urlSlots))fail("acquisition-contract.result-too-large");const requestedUrl=projectUrl(value.requestedUrl,normalizedLimits.maxCanonicalUrlBytes);const finalUrl=value.finalUrl===null?null:projectUrl(value.finalUrl,normalizedLimits.maxCanonicalUrlBytes);const redirects=value.redirectWitnesses.map((item)=>deepFreeze({ordinal:item.ordinal,status:item.status,from:projectUrl(item.fromUrl,normalizedLimits.maxCanonicalUrlBytes),to:projectUrl(item.toUrl,normalizedLimits.maxCanonicalUrlBytes)}));return deepFreeze({outcome:value.outcome,code:settlementCode(value),requestedUrl,finalUrl,redirects,httpStatus:value.httpStatus,encodedBytes:value.encodedBytes,decodedBytes:value.decodedBytes,responsePayloadSha256:value.responsePayloadSha256,responseHeadersSha256:value.responseHeadersSha256,retrievedAt:timestamp(value.settledAt,"acquisition-contract.invalid-input")}); }
export function createBlockedTraceSettlementInternal(input:Readonly<{requestedUrl:string;retrievedAt:string;limits:AcquisitionTraceProjectionLimitsInternal;}>):AcquisitionTraceSettlement { preflightSafeStructure(input,PUBLIC_STRUCTURE,"acquisition-contract.invalid-input","acquisition-contract.result-too-large");const descriptors=descriptorRecord(input,["requestedUrl","retrievedAt","limits"],"acquisition-contract.invalid-input");if(!descriptors.limits)fail("acquisition-contract.invalid-input");validateProjectionLimits(descriptors.limits.value);const data=snapshot<Record<string,unknown>>(input,PUBLIC_STRUCTURE,"acquisition-contract.invalid-input","acquisition-contract.result-too-large");const v=requireClosed(data,["requestedUrl","retrievedAt","limits"],"acquisition-contract.invalid-input");const limits=validateProjectionLimits(v.limits);const raw=stringValue(v.requestedUrl,"acquisition-contract.invalid-input")!;const origin=new URL(canonicalTransportUrl(raw,limits.maxCanonicalUrlBytes)).origin;if(!["https://api.crossref.org","https://api.openalex.org","https://eutils.ncbi.nlm.nih.gov","https://www.ncbi.nlm.nih.gov"].includes(origin))fail("acquisition-contract.invalid-input");return deepFreeze({outcome:"blocked",code:"sink-blocked",requestedUrl:projectUrl(raw,limits.maxCanonicalUrlBytes),finalUrl:null,redirects:[],httpStatus:null,encodedBytes:0,decodedBytes:0,responsePayloadSha256:null,responseHeadersSha256:null,retrievedAt:timestamp(v.retrievedAt,"acquisition-contract.invalid-input")}); }

function validateProjectedTraceUrl(value:unknown,allowedOrigins:readonly string[]):AcquisitionTraceUrl {
  const v=requireClosed(value,["canonicalUrl","canonicalUrlSha256"],"acquisition-contract.invalid-input");
  const canonicalUrl=stringValue(v.canonicalUrl,"acquisition-contract.invalid-input")!;if(Buffer.byteLength(canonicalJson(canonicalUrl),"utf8")>16_384)fail("acquisition-contract.result-too-large");const canonicalUrlSha256=hashValue(v.canonicalUrlSha256,"acquisition-contract.invalid-key");
  let url:URL;try{url=new URL(canonicalUrl);}catch{return fail("acquisition-contract.invalid-input");}
  if(url.protocol!=="https:"||!allowedOrigins.includes(url.origin)||url.username!==""||url.password!==""||url.hash!==""||(url.search!==""&&url.search!=="?redacted")||/%[0-9a-f]*[a-f][0-9a-f]*/u.test(url.pathname)||url.href!==canonicalUrl)fail("acquisition-contract.invalid-input");
  return deepFreeze({canonicalUrl,canonicalUrlSha256});
}
function validateTraceSettlement(value:unknown,provider:AcquisitionProvider,operation:AcquisitionOperation,endpointClass:string):AcquisitionTraceSettlement {
  const v=requireClosed(value,["outcome","code","requestedUrl","finalUrl","redirects","httpStatus","encodedBytes","decodedBytes","responsePayloadSha256","responseHeadersSha256","retrievedAt"],"acquisition-contract.invalid-input");
  const outcome=oneOf(v.outcome,["success","failure","cancelled","blocked"] as const,"acquisition-contract.invalid-input");const code=oneOf(v.code,["success","availability","terminal-provider","http-retryable","http-terminal","invalid-response","sink-blocked","cancelled","deadline-exceeded","security-policy"] as const,"acquisition-contract.invalid-input");
  stringValue(endpointClass,"acquisition-contract.invalid-input");void operation;const allowedOrigins=providerOrigins(provider);const requestedUrl=validateProjectedTraceUrl(v.requestedUrl,allowedOrigins);const finalUrl=v.finalUrl===null?null:validateProjectedTraceUrl(v.finalUrl,allowedOrigins);
  preflightArray(v.redirects,5,"acquisition-contract.invalid-input","acquisition-contract.result-too-large");let prior=requestedUrl;let ordinal=1;
  const redirects=(v.redirects as unknown[]).map((item)=>{const r=requireClosed(item,["ordinal","status","from","to"],"acquisition-contract.invalid-input");if(r.ordinal!==ordinal||![301,302,303,307,308].includes(r.status as number))fail("acquisition-contract.invalid-input");ordinal+=1;const from=validateProjectedTraceUrl(r.from,allowedOrigins);const to=validateProjectedTraceUrl(r.to,allowedOrigins);if(from.canonicalUrl!==prior.canonicalUrl||from.canonicalUrlSha256!==prior.canonicalUrlSha256)fail("acquisition-contract.invalid-input");prior=to;return deepFreeze({ordinal:r.ordinal as number,status:r.status as 301|302|303|307|308,from,to});});
  const expectedFinal=redirects.length===0?requestedUrl:prior;if(finalUrl!==null&&(finalUrl.canonicalUrl!==expectedFinal.canonicalUrl||finalUrl.canonicalUrlSha256!==expectedFinal.canonicalUrlSha256))fail("acquisition-contract.invalid-input");if(finalUrl===null&&redirects.length!==0)fail("acquisition-contract.invalid-input");
  if(v.httpStatus!==null)integer(v.httpStatus,999,"acquisition-contract.invalid-input");nonnegative(v.encodedBytes,"acquisition-contract.invalid-input");nonnegative(v.decodedBytes,"acquisition-contract.invalid-input");if(v.responsePayloadSha256!==null)hashValue(v.responsePayloadSha256,"acquisition-contract.invalid-key");if(v.responseHeadersSha256!==null)hashValue(v.responseHeadersSha256,"acquisition-contract.invalid-key");timestamp(v.retrievedAt,"acquisition-contract.invalid-input");
  if((outcome==="success"&&(code!=="success"||finalUrl===null))||(outcome==="blocked"&&code!=="sink-blocked")||(outcome==="cancelled"&&code!=="cancelled"&&code!=="deadline-exceeded")||(outcome==="failure"&&(code==="success"||code==="sink-blocked"||code==="cancelled"||code==="deadline-exceeded")))fail("acquisition-contract.invalid-input");
  if(outcome==="blocked"&&(finalUrl!==null||redirects.length!==0||v.httpStatus!==null||v.encodedBytes!==0||v.decodedBytes!==0||v.responsePayloadSha256!==null||v.responseHeadersSha256!==null))fail("acquisition-contract.invalid-input");
  return deepFreeze({...v,requestedUrl,finalUrl,redirects} as unknown as AcquisitionTraceSettlement);
}
function preflightTraceRedirects(input:unknown):void { const outer=descriptorRecord(input,["provider","operation","endpointClass","partitionKey","partitionKeySha256","accessLevel","settlement","warnings","provenanceStatus","schemaVersion","traceKey"],"acquisition-contract.invalid-input");const settlement=outer.settlement;if(!settlement||!("value" in settlement))fail("acquisition-contract.invalid-input");const fields=["outcome","code","requestedUrl","finalUrl","redirects","httpStatus","encodedBytes","decodedBytes","responsePayloadSha256","responseHeadersSha256","retrievedAt"];const descriptors=descriptorRecord(settlement.value,fields,"acquisition-contract.invalid-input");for(const field of fields)if(!descriptors[field])fail("acquisition-contract.invalid-input");descriptorRecord(descriptors.requestedUrl!.value,["canonicalUrl","canonicalUrlSha256"],"acquisition-contract.invalid-input");if(descriptors.finalUrl!.value!==null)descriptorRecord(descriptors.finalUrl!.value,["canonicalUrl","canonicalUrlSha256"],"acquisition-contract.invalid-input");const redirects=descriptors.redirects!;preflightArray(redirects.value,5,"acquisition-contract.invalid-input","acquisition-contract.result-too-large");preflightSafeStructure(input,PUBLIC_STRUCTURE,"acquisition-contract.invalid-input","acquisition-contract.result-too-large");for(const redirect of redirects.value as unknown[]){const redirectDescriptors=descriptorRecord(redirect,["ordinal","status","from","to"],"acquisition-contract.invalid-input");if(!redirectDescriptors.from||!redirectDescriptors.to)fail("acquisition-contract.invalid-input");descriptorRecord(redirectDescriptors.from.value,["canonicalUrl","canonicalUrlSha256"],"acquisition-contract.invalid-input");descriptorRecord(redirectDescriptors.to.value,["canonicalUrl","canonicalUrlSha256"],"acquisition-contract.invalid-input");}if(!outer.warnings)fail("acquisition-contract.invalid-input");preflightArray(outer.warnings.value,1_000,"acquisition-contract.invalid-input","acquisition-contract.result-too-large");for(const warning of outer.warnings.value as unknown[])descriptorRecord(warning,["code","recordOrdinal","field"],"acquisition-contract.invalid-input"); }
...
export function createAcquisitionTrace(input:AcquisitionTraceInput):AcquisitionTrace { preflightTraceRedirects(input);const data=snapshot<Record<string,unknown>>(input,PUBLIC_STRUCTURE,"acquisition-contract.invalid-input","acquisition-contract.result-too-large");if(Object.hasOwn(data,"provenanceStatus"))fail("acquisition-contract.invalid-provenance");const keys=["provider","operation","endpointClass","partitionKey","partitionKeySha256","accessLevel","settlement","warnings"];const v=requireClosed(data,keys,"acquisition-contract.invalid-input");const provider=oneOf(v.provider,PROVIDERS,"acquisition-contract.invalid-input");const operation=oneOf(v.operation,OPERATIONS,"acquisition-contract.invalid-input");const endpointClass=stringValue(v.endpointClass,"acquisition-contract.invalid-input")!;const partitionKey=keyValue(v.partitionKey,KEY_PARTITION,"acquisition-contract.invalid-key") as `partition-v1-${string}`;const partitionKeySha256=hashValue(v.partitionKeySha256,"acquisition-contract.invalid-key");if(partitionKey!==`partition-v1-${partitionKeySha256}`)fail("acquisition-contract.invalid-key");const registeredPartition=resolveIndexedPartition(partitionKey,partitionKeySha256);if(provider!==registeredPartition.provider||operation!==registeredPartition.operation||endpointClass!==registeredPartition.endpointClass)fail("acquisition-contract.invalid-key");const accessLevel=v.accessLevel===null?null:oneOf(v.accessLevel,ACCESS,"acquisition-contract.invalid-input");const settlement=validateTraceSettlement(v.settlement,provider,operation,endpointClass);const expectedRequestedUrl=projectUrl(registeredPartition.url,16_384);if(settlement.requestedUrl.canonicalUrl!==expectedRequestedUrl.canonicalUrl||settlement.requestedUrl.canonicalUrlSha256!==expectedRequestedUrl.canonicalUrlSha256)fail("acquisition-contract.invalid-key");if(!Array.isArray(v.warnings))fail("acquisition-contract.invalid-input");const warnings=v.warnings.map((item)=>{const w=requireClosed(item,["code","recordOrdinal","field"],"acquisition-contract.invalid-input");return deepFreeze({code:oneOf(w.code,["provider.invalid-url-omitted","provider.invalid-identifier-omitted","provider.followup-unavailable"] as const,"acquisition-contract.invalid-input"),recordOrdinal:w.recordOrdinal===null?null:nonnegative(w.recordOrdinal,"acquisition-contract.invalid-input"),field:oneOf(w.field,["URL","primary_location.landing_page_url","doi","ids.doi","ids.pmid","ids.pmcid","elink"] as const,"acquisition-contract.invalid-input")});});const body={provider,operation,endpointClass,partitionKey,partitionKeySha256,accessLevel,settlement,warnings};const traceKey=prefixHash("trace-v1-",tracePreimage(body)) as `trace-v1-${string}`;return deepFreeze({schemaVersion:1,traceKey,provenanceStatus:"uncommitted" as const,...body}); }

function preflightCandidateShape(input:unknown):void { preflightSafeStructure(input,PUBLIC_STRUCTURE,"acquisition-contract.invalid-input","acquisition-contract.result-too-large");const allowed=["provider","endpointClass","providerRecordId","providerOrdinal","partitionKeySha256","responsePayloadSha256","traceKey","identifiers","canonicalUrl","title","authors","containerTitle","publisher","published","publicationType","accessLevel","abstractText","schemaVersion","candidateKey","provenanceStatus"];const descriptors=descriptorRecord(input,allowed,"acquisition-contract.invalid-input");if(!descriptors.identifiers||!descriptors.authors||!descriptors.published)fail("acquisition-contract.invalid-input");descriptorRecord(descriptors.identifiers.value,["doi","pmid","pmcid"],"acquisition-contract.invalid-input");preflightArray(descriptors.authors.value,1_024,"acquisition-contract.invalid-input","acquisition-contract.result-too-large");for(const author of descriptors.authors.value as unknown[])descriptorRecord(author,["family","given","literal","orcid"],"acquisition-contract.invalid-input");descriptorRecord(descriptors.published.value,["date","precision"],"acquisition-contract.invalid-input"); }
...
function computeCandidateGroups(candidates:readonly AcademicCandidate[]):readonly AcademicCandidateGroup[] {
  const ordered=<T>(values:readonly T[],key:(value:T)=>string):T[]=>stableSortByCodeUnitKeyInternal(values,key,{maxItems:3_000});
  const parents=candidates.map((_,index)=>index);const find=(index:number):number=>{let current=index;while(parents[current]!==current)current=parents[current]!;while(parents[index]!==index){const next=parents[index]!;parents[index]=current;index=next;}return current;};const union=(left:number,right:number):void=>{const a=find(left);const b=find(right);if(a!==b)parents[Math.max(a,b)]=Math.min(a,b);};
  const owners=new Map<string,number>();for(let index=0;index<candidates.length;index+=1){const candidate=candidates[index]!;const references=[candidate.identifiers.doi===null?null:`doi:${candidate.identifiers.doi}`,candidate.identifiers.pmid===null?null:`pmid:${candidate.identifiers.pmid}`,candidate.identifiers.pmcid===null?null:`pmcid:${candidate.identifiers.pmcid}`,candidate.canonicalUrl===null?null:`url:${candidate.canonicalUrl}`];for(const reference of references){if(reference===null)continue;const owner=owners.get(reference);if(owner===undefined)owners.set(reference,index);else union(index,owner);}}
  const components=new Map<number,AcademicCandidate[]>();for(let index=0;index<candidates.length;index+=1){const root=find(index);const values=components.get(root)??[];values.push(candidates[index]!);components.set(root,values);}
  const rank={doi:"0",pmid:"1",pmcid:"2"} as const;const groups:AcademicCandidateGroup[]=[];
  for(const values of components.values()){
    const candidateKeys=ordered(values.map(({candidateKey})=>candidateKey),(key)=>key);const sets={doi:new Set<string>(),pmid:new Set<string>(),pmcid:new Set<string>()};for(const value of values)for(const kind of ["doi","pmid","pmcid"] as const){const identifier=value.identifiers[kind];if(identifier!==null)sets[kind].add(identifier);}
    const conflicting=(["doi","pmid","pmcid"] as const).some((kind)=>sets[kind].size>1);const sameUrl=values[0]!.canonicalUrl!==null&&values.every((value)=>value.canonicalUrl===values[0]!.canonicalUrl);const common=(["doi","pmid","pmcid"] as const).find((kind)=>{const first=values[0]!.identifiers[kind];return first!==null&&values.every((value)=>value.identifiers[kind]===first);});
    let identityKind:AcademicCandidateGroup["identityKind"];let identityValue:string;let status:AcademicCandidateGroup["status"];
    if(sameUrl&&conflicting){identityKind="canonical-url";identityValue=values[0]!.canonicalUrl!;status="ambiguous";}else if(common!==undefined){identityKind=common;identityValue=values[0]!.identifiers[common]!;status=conflicting?"ambiguous":"compatible";}else if(sameUrl){identityKind="canonical-url";identityValue=values[0]!.canonicalUrl!;status="compatible";}else if(values.length===1){const first=(["doi","pmid","pmcid"] as const).find((kind)=>sets[kind].size===1);if(first!==undefined){identityKind=first;identityValue=[...sets[first]][0]!;}else if(values[0]!.canonicalUrl!==null){identityKind="canonical-url";identityValue=values[0]!.canonicalUrl;}else{identityKind="provider-result";identityValue=values[0]!.candidateKey;}status="compatible";}else{const keys=ordered((["doi","pmid","pmcid"] as const).flatMap((kind)=>[...sets[kind]].map((value)=>({kind,value}))),(item)=>`${rank[item.kind]}${item.value}`);identityKind=keys[0]!.kind;identityValue=keys[0]!.value;status="ambiguous";}
    const identityValueSha256=sha256Hex(canonicalJson(identityValue));const groupKey=prefixHash("candidate-group-v1-",{schemaVersion:1,identityKind,identityValueSha256,sortedCandidateKeys:candidateKeys,status}) as `candidate-group-v1-${string}`;groups.push(deepFreeze({groupKey,identityKind,identityValueSha256,status,candidateKeys}));
  }
  return deepFreeze(ordered(groups,(group)=>group.groupKey));
}
function preflightResultCollections(input:unknown):void { const keys=["schemaVersion","provenanceStatus","status","normalizedQueries","partitions","candidates","candidateGroups","documents","traces","failures","optionsSha256"];const descriptors=descriptorRecord(input,keys,"acquisition-contract.invalid-input");for(const [field,maximum] of [["normalizedQueries",16],["candidates",1_000],["candidateGroups",1_000],["documents",1_000],["traces",64],["failures",64]] as const){const descriptor=descriptors[field];if(!descriptor)fail("acquisition-contract.invalid-input");preflightArray(descriptor.value,maximum,"acquisition-contract.invalid-input","acquisition-contract.result-too-large");}if(!descriptors.partitions)fail("acquisition-contract.invalid-input");descriptorRecord(descriptors.partitions.value,["requested","dispatched","succeeded","failed","blocked"],"acquisition-contract.invalid-input");for(const candidate of descriptors.candidates!.value as unknown[])preflightCandidateShape(candidate);for(const group of descriptors.candidateGroups!.value as unknown[]){const groupDescriptors=descriptorRecord(group,["groupKey","identityKind","identityValueSha256","status","candidateKeys"],"acquisition-contract.invalid-input");if(!groupDescriptors.candidateKeys)fail("acquisition-contract.invalid-input");preflightArray(groupDescriptors.candidateKeys.value,1_000,"acquisition-contract.invalid-input","acquisition-contract.result-too-large");}for(const document of descriptors.documents!.value as unknown[])preflightDocumentShape(document);for(const trace of descriptors.traces!.value as unknown[])preflightTraceRedirects(trace);for(const failure of descriptors.failures!.value as unknown[])descriptorRecord(failure,["provider","operation","partitionKey","partitionKeySha256","code","retryable"],"acquisition-contract.invalid-input");preflightSafeStructure(input,PUBLIC_STRUCTURE,"acquisition-contract.invalid-input","acquisition-contract.result-too-large");}
function validateAcademicAcquisitionResultImpl(input:unknown):AcademicAcquisitionResult { preflightResultCollections(input);const data=snapshot<Record<string,unknown>>(input,PUBLIC_STRUCTURE,"acquisition-contract.invalid-input","acquisition-contract.result-too-large");if(Buffer.byteLength(canonicalJson(data),"utf8")>67_108_864)fail("acquisition-contract.result-too-large");const v=requireClosed(data,["schemaVersion","provenanceStatus","status","normalizedQueries","partitions","candidates","candidateGroups","documents","traces","failures","optionsSha256"],"acquisition-contract.invalid-input");if(v.schemaVersion!==1)fail("acquisition-contract.invalid-input");if(v.provenanceStatus!=="uncommitted")fail("acquisition-contract.invalid-provenance");const status=oneOf(v.status,["complete","partial"] as const,"acquisition-contract.invalid-input");if(!Array.isArray(v.normalizedQueries))fail("acquisition-contract.invalid-input");const normalizedQueries=v.normalizedQueries.map((item)=>stringValue(item,"acquisition-contract.invalid-input")!);const p=requireClosed(v.partitions,["requested","dispatched","succeeded","failed","blocked"],"acquisition-contract.invalid-input");const partitions={requested:nonnegative(p.requested,"acquisition-contract.invalid-input"),dispatched:nonnegative(p.dispatched,"acquisition-contract.invalid-input"),succeeded:nonnegative(p.succeeded,"acquisition-contract.invalid-input"),failed:nonnegative(p.failed,"acquisition-contract.invalid-input"),blocked:nonnegative(p.blocked,"acquisition-contract.invalid-input")};if(checkedAdd(partitions.dispatched,partitions.blocked,"acquisition-contract.invalid-input")!==partitions.requested||partitions.blocked>partitions.failed||checkedAdd(partitions.succeeded,partitions.failed-partitions.blocked,"acquisition-contract.invalid-input")!==partitions.dispatched)fail("acquisition-contract.invalid-input");if(!Array.isArray(v.traces)||!Array.isArray(v.candidates)||!Array.isArray(v.documents)||!Array.isArray(v.candidateGroups)||!Array.isArray(v.failures))fail("acquisition-contract.invalid-input");const traces=v.traces.map(rebuildTrace);const traceMap=new Map(traces.map((item)=>[item.traceKey,item]));const tracePartitionMap=new Map(traces.map((item)=>[item.partitionKey,item]));if(traceMap.size!==traces.length||tracePartitionMap.size!==traces.length)fail("acquisition-contract.invalid-key");const candidates=v.candidates.map(rebuildCandidate);const candidateMap=new Map(candidates.map((item)=>[item.candidateKey,item]));if(candidateMap.size!==candidates.length)fail("acquisition-contract.invalid-key");for(const item of candidates){const t=traceMap.get(item.traceKey);if(!t||t.partitionKeySha256!==item.partitionKeySha256||t.provider!==item.provider||t.endpointClass!==item.endpointClass||t.settlement.responsePayloadSha256!==item.responsePayloadSha256)fail("acquisition-contract.invalid-key");}const documents=v.documents.map(rebuildDocument);const documentMap=new Map(documents.map((item)=>[item.documentKey,item]));if(documentMap.size!==documents.length)fail("acquisition-contract.invalid-key");for(const item of documents){const c=candidateMap.get(item.candidateKey);const t=traceMap.get(item.traceKey);if(!c||!t||c.provider!=="pmc"||c.identifiers.pmcid!==item.pmcid||c.traceKey!==item.traceKey||c.partitionKeySha256!==item.partitionKeySha256||c.responsePayloadSha256!==item.responsePayloadSha256||t.provider!=="pmc"||t.operation!=="fetch"||t.partitionKeySha256!==item.partitionKeySha256||t.settlement.responsePayloadSha256!==item.responsePayloadSha256||ACCESS.indexOf(t.accessLevel??"metadata-only")<ACCESS.indexOf(item.accessLevel)||ACCESS.indexOf(c.accessLevel)<ACCESS.indexOf(item.accessLevel))fail("acquisition-contract.invalid-key");}const candidateGroups=computeCandidateGroups(candidates);if(canonicalJson(v.candidateGroups)!==canonicalJson(candidateGroups))fail("acquisition-contract.invalid-key");const failures=v.failures.map((item)=>{const f=requireClosed(item,["provider","operation","partitionKey","partitionKeySha256","code","retryable"],"acquisition-contract.invalid-input");const partitionKey=keyValue(f.partitionKey,KEY_PARTITION,"acquisition-contract.invalid-key");const partitionKeySha256=hashValue(f.partitionKeySha256,"acquisition-contract.invalid-key");if(partitionKey!==`partition-v1-${partitionKeySha256}`)fail("acquisition-contract.invalid-key");if(typeof f.retryable!=="boolean")fail("acquisition-contract.invalid-input");const provider=oneOf(f.provider,PROVIDERS,"acquisition-contract.invalid-input");const operation=oneOf(f.operation,OPERATIONS,"acquisition-contract.invalid-input");const trace=tracePartitionMap.get(partitionKey as `partition-v1-${string}`);if(trace&&(trace.provider!==provider||trace.operation!==operation||trace.partitionKeySha256!==partitionKeySha256))fail("acquisition-contract.invalid-key");return deepFreeze({provider,operation,partitionKey:partitionKey as `partition-v1-${string}`,partitionKeySha256,code:oneOf(f.code,["provider.retryable","provider.terminal","provider.invalid-response","provider.unsupported","sink.failed"] as const,"acquisition-contract.invalid-input"),retryable:f.retryable});});const complete=partitions.failed===0&&failures.length===0;if((status==="complete")!==complete)fail("acquisition-contract.invalid-input");const optionsSha256=hashValue(v.optionsSha256,"acquisition-contract.invalid-key");return deepFreeze({schemaVersion:1,provenanceStatus:"uncommitted" as const,status,normalizedQueries,partitions,candidates,candidateGroups,documents,traces,failures,optionsSha256}); }
export function validateAcademicAcquisitionResult(input:unknown):AcademicAcquisitionResult { return validateAcademicAcquisitionResultImpl(input); }

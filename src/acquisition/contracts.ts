...
function assertBoundedStructure(value:unknown,limits:StructuralLimits):void {
  ...
  visitShape(value,0);
  const addCanonical=(scalar:unknown,isStringOrKey:boolean):void=>{const bytes=Buffer.byteLength(canonicalJson(scalar),"utf8");if((isStringOrKey&&bytes>limits.maxStringBytes)||bytes>limits.maxScalarBytes)throw new StructuralLimitError("limit");};
  const visitCanonical=(item:unknown):void=>{if(typeof item==="string"){addCanonical(item,true);return;}if(item===null||typeof item==="boolean"||typeof item==="number"){addCanonical(item,false);return;}const descriptors=descriptorsByObject.get(item as object);if(!descriptors)throw new StructuralLimitError("unsafe");const array=Array.isArray(item);for(const key of Reflect.ownKeys(descriptors) as string[]){if(array&&key==="length")continue;addCanonical(key,true);visitCanonical(descriptors[key]!.value);}};
...
export interface AcquisitionTraceProjectionLimitsInternal { readonly maxCanonicalUrlBytes:number;readonly maxRedirects:number;readonly maxDecodedBodyBytes:number; }
...
function validateProjectionLimits(value:unknown):AcquisitionTraceProjectionLimitsInternal { const data=snapshot<Record<string,unknown>>(value,OPTION_STRUCTURE,"acquisition-contract.invalid-input","acquisition-contract.result-too-large");const v=requireClosed(data,["maxCanonicalUrlBytes","maxRedirects","maxDecodedBodyBytes"],"acquisition-contract.invalid-input");return {maxCanonicalUrlBytes:integer(v.maxCanonicalUrlBytes,16_384,"acquisition-contract.invalid-input"),maxRedirects:integer(v.maxRedirects,5,"acquisition-contract.invalid-input"),maxDecodedBodyBytes:integer(v.maxDecodedBodyBytes,67_108_864,"acquisition-contract.invalid-input")}; }
...
function validateSettlement(value:unknown,partition:ProviderRequestPartitionSnapshotInternal,limits:AcquisitionTraceProjectionLimitsInternal):TransportSettlement {
  ...
  const payload=descriptors.payloadUtf8!.value;const outcome=descriptors.outcome!.value;
  if(outcome==="success"){
    if(typeof payload!=="string")fail("acquisition-contract.invalid-input");if(payload.length>limits.maxDecodedBodyBytes)fail("acquisition-contract.result-too-large");const payloadBytes=Buffer.byteLength(payload,"utf8");if(payloadBytes>limits.maxDecodedBodyBytes)fail("acquisition-contract.result-too-large");if(descriptors.decodedBytes!.value!==payloadBytes)fail("acquisition-contract.invalid-input");
  ...
  if(v.httpStatus!==null)integer(v.httpStatus,999,"acquisition-contract.invalid-input");nonnegative(v.encodedBytes,"acquisition-contract.invalid-input");const decodedBytes=nonnegative(v.decodedBytes,"acquisition-contract.invalid-input");if(decodedBytes>limits.maxDecodedBodyBytes)fail("acquisition-contract.result-too-large");if(v.responsePayloadSha256!==null)hashValue(v.responsePayloadSha256,"acquisition-contract.invalid-input");if(v.responseHeadersSha256!==null)hashValue(v.responseHeadersSha256,"acquisition-contract.invalid-input");
...
export interface AcquisitionGroupingDiagnosticsInternal { candidateCount:number;identityReferences:number;unionFindOperations:number;orderingCodeUnits:number; }
function computeCandidateGroups(candidates:readonly AcademicCandidate[],diagnostics?:AcquisitionGroupingDiagnosticsInternal):readonly AcademicCandidateGroup[] {
  const counts=diagnostics??{candidateCount:0,identityReferences:0,unionFindOperations:0,orderingCodeUnits:0};counts.candidateCount=candidates.length;counts.identityReferences=0;counts.unionFindOperations=0;counts.orderingCodeUnits=0;
  const ordered=<T>(values:readonly T[],key:(value:T)=>string):T[]=>{for(const value of values)counts.orderingCodeUnits+=key(value).length;return stableSortByCodeUnitKeyInternal(values,key,{maxItems:3_000});};
  const parents=candidates.map((_,index)=>index);const find=(index:number):number=>{counts.unionFindOperations+=1;let current=index;while(parents[current]!==current){counts.unionFindOperations+=1;current=parents[current]!;}while(parents[index]!==index){const next=parents[index]!;parents[index]=current;index=next;}return current;};const union=(left:number,right:number):void=>{const a=find(left);const b=find(right);if(a!==b)parents[Math.max(a,b)]=Math.min(a,b);};
  const owners=new Map<string,number>();for(let index=0;index<candidates.length;index+=1){const candidate=candidates[index]!;const references=[candidate.identifiers.doi===null?null:`doi:${candidate.identifiers.doi}`,candidate.identifiers.pmid===null?null:`pmid:${candidate.identifiers.pmid}`,candidate.identifiers.pmcid===null?null:`pmcid:${candidate.identifiers.pmcid}`,candidate.canonicalUrl===null?null:`url:${candidate.canonicalUrl}`];for(const reference of references){if(reference===null)continue;counts.identityReferences+=1;const owner=owners.get(reference);if(owner===undefined)owners.set(reference,index);else union(index,owner);}}
...
export function validateAcademicAcquisitionResultWithDiagnosticsInternal(input:unknown,diagnostics:AcquisitionGroupingDiagnosticsInternal):AcademicAcquisitionResult { if(!plain(diagnostics)||!Object.isExtensible(diagnostics))fail("acquisition-contract.invalid-capability");const descriptors=Object.getOwnPropertyDescriptors(diagnostics);if(Reflect.ownKeys(descriptors).some((key)=>typeof key!=="string"||!["candidateCount","identityReferences","unionFindOperations","orderingCodeUnits"].includes(key)||!("value" in descriptors[key]!)||descriptors[key]!.writable!==true))fail("acquisition-contract.invalid-capability");return validateAcademicAcquisitionResultImpl(input,diagnostics); }

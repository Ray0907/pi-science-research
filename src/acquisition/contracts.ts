...
function assertBoundedStructure(value:unknown,limits:StructuralLimits):void {
  ...
  visitShape(value,0);
  let canonicalScalarBytes=0;const addCanonical=(scalar:unknown,isStringOrKey:boolean):void=>{const bytes=Buffer.byteLength(canonicalJson(scalar),"utf8");canonicalScalarBytes+=bytes;if(!Number.isSafeInteger(canonicalScalarBytes)||(isStringOrKey&&bytes>limits.maxStringBytes)||canonicalScalarBytes>limits.maxScalarBytes)throw new StructuralLimitError("limit");};
...
export function createProviderPartitionKey(input:Readonly<{provider:AcquisitionProvider;operation:AcquisitionOperation;endpointClass:string;target:"crossref"|"openalex"|"ncbi";normalizedInput:ProviderNormalizedInput;requestedUrl:string;}>):Readonly<{partitionKey:`partition-v1-${string}`;partitionKeySha256:string}> {
  ...
  const normalizedInput=validateNormalizedInput(v.normalizedInput);const requestedUrl=fixedUrl(v.requestedUrl,target);if(new URL(requestedUrl).origin!==expectedProviderOrigin(provider,endpointClass,operation))fail("acquisition-contract.invalid-input");const preimage={schemaVersion:1,provider,operation,endpointClass,target,normalizedInput,requestedUrl};const partitionKeySha256=sha256Hex(canonicalJson(preimage));return deepFreeze({partitionKey:`partition-v1-${partitionKeySha256}`,partitionKeySha256});
}
...
function expectedProviderOrigin(provider:AcquisitionProvider,endpointClass:string,operation:AcquisitionOperation):string {
  if(endpointClass.length===0||/[\u0000-\u001f\u007f]/u.test(endpointClass))fail("acquisition-contract.invalid-input");
  const compatible=provider==="crossref"||provider==="openalex"
    ? (operation==="search"?/search/u.test(endpointClass):/fetch/u.test(endpointClass))
    : provider==="pubmed" ? /(?:esearch|esummary|elink)/u.test(endpointClass)
    : /bioc/u.test(endpointClass);
  if(!compatible)fail("acquisition-contract.invalid-input");
  return provider==="crossref"?"https://api.crossref.org":provider==="openalex"?"https://api.openalex.org":provider==="pubmed"?"https://eutils.ncbi.nlm.nih.gov":"https://www.ncbi.nlm.nih.gov";
}
...
function validateSettlement(value:unknown):TransportSettlement {
  const data=snapshot<Record<string,unknown>>(value,PUBLIC_STRUCTURE,"acquisition-contract.invalid-input","acquisition-contract.input-too-large");
  ...
  const v=requireClosed(data,common,"acquisition-contract.invalid-input");
  if(v.schemaVersion!==1)fail("acquisition-contract.invalid-input");
  keyValue(v.partitionKey,KEY_PARTITION,"acquisition-contract.invalid-key");hashValue(v.partitionKeySha256,"acquisition-contract.invalid-key");
  if(v.partitionKey!==`partition-v1-${v.partitionKeySha256}`)fail("acquisition-contract.invalid-key");
  const provider=oneOf(v.provider,PROVIDERS,"acquisition-contract.invalid-input");
  const operation=oneOf(v.operation,OPERATIONS,"acquisition-contract.invalid-input");
  const endpointClass=stringValue(v.endpointClass,"acquisition-contract.invalid-input")!;
  const expectedOrigin=expectedProviderOrigin(provider,endpointClass,operation);
  const normalizedOrigin=stringValue(v.normalizedOrigin,"acquisition-contract.invalid-input")!;
  if(normalizedOrigin!==expectedOrigin)fail("acquisition-contract.invalid-input");
  const requestedUrl=canonicalTransportUrl(v.requestedUrl,16_384);if(new URL(requestedUrl).origin!==expectedOrigin)fail("acquisition-contract.invalid-input");
  const finalUrl=v.finalUrl===null?null:canonicalTransportUrl(v.finalUrl,16_384);if(finalUrl!==null&&new URL(finalUrl).origin!==expectedOrigin)fail("acquisition-contract.invalid-input");
  if(!Array.isArray(v.redirectWitnesses))fail("acquisition-contract.invalid-input");
  let priorUrl=requestedUrl;let expectedOrdinal=1;
  for(const item of v.redirectWitnesses){
    const r=requireClosed(item,["ordinal","status","fromUrl","fromOrigin","toUrl","toOrigin"],"acquisition-contract.invalid-input");
    if(r.ordinal!==expectedOrdinal||![301,302,303,307,308].includes(r.status as number))fail("acquisition-contract.invalid-input");expectedOrdinal+=1;
    const fromUrl=canonicalTransportUrl(r.fromUrl,16_384);const toUrl=canonicalTransportUrl(r.toUrl,16_384);
    const fromOrigin=stringValue(r.fromOrigin,"acquisition-contract.invalid-input")!;const toOrigin=stringValue(r.toOrigin,"acquisition-contract.invalid-input")!;
    if(fromUrl!==priorUrl||new URL(fromUrl).origin!==fromOrigin||new URL(toUrl).origin!==toOrigin||fromOrigin!==expectedOrigin||toOrigin!==expectedOrigin)fail("acquisition-contract.invalid-input");
    priorUrl=toUrl;
  }
  if(finalUrl===null){if(v.redirectWitnesses.length!==0)fail("acquisition-contract.invalid-input");}
  else if(finalUrl!==(v.redirectWitnesses.length===0?requestedUrl:priorUrl))fail("acquisition-contract.invalid-input");
  if(v.httpStatus!==null)integer(v.httpStatus,999,"acquisition-contract.invalid-input");nonnegative(v.encodedBytes,"acquisition-contract.invalid-input");nonnegative(v.decodedBytes,"acquisition-contract.invalid-input");
  if(v.responsePayloadSha256!==null)hashValue(v.responsePayloadSha256,"acquisition-contract.invalid-input");if(v.responseHeadersSha256!==null)hashValue(v.responseHeadersSha256,"acquisition-contract.invalid-input");
  const headers=requireClosed(v.responseHeaders,["contentType","retryAfter","etag","lastModified"],"acquisition-contract.invalid-input");for(const key of ["contentType","retryAfter","etag","lastModified"])stringValue(headers[key],"acquisition-contract.invalid-input",true);
  if(v.connectedPeer!==null){const peer=requireClosed(v.connectedPeer,["address","family"],"acquisition-contract.invalid-input");stringValue(peer.address,"acquisition-contract.invalid-input");if(peer.family!==4&&peer.family!==6)fail("acquisition-contract.invalid-input");}
  timestamp(v.startedAt,"acquisition-contract.invalid-input");timestamp(v.settledAt,"acquisition-contract.invalid-input");
  if(v.outcome==="success"){if(v.failureCode!==null||typeof v.payloadUtf8!=="string"||v.responsePayloadSha256===null||finalUrl===null)fail("acquisition-contract.invalid-input");}
  else if(v.outcome==="failure"){oneOf(v.failureCode,TRANSPORT_FAILURES,"acquisition-contract.invalid-input");if(v.failureCode==="transport.cancelled"||v.failureCode==="transport.deadline-exceeded"||v.payloadUtf8!==null)fail("acquisition-contract.invalid-input");}
  else if(v.outcome==="cancelled"){if(!["transport.cancelled","transport.deadline-exceeded"].includes(v.failureCode as string)||v.payloadUtf8!==null)fail("acquisition-contract.invalid-input");}
  else fail("acquisition-contract.invalid-input");
  return deepFreeze(data) as unknown as TransportSettlement;
}
...
function validateProjectedTraceUrl(value:unknown,expectedOrigin:string):AcquisitionTraceUrl {
  const v=requireClosed(value,["canonicalUrl","canonicalUrlSha256"],"acquisition-contract.invalid-input");
  const canonicalUrl=stringValue(v.canonicalUrl,"acquisition-contract.invalid-input")!;const canonicalUrlSha256=hashValue(v.canonicalUrlSha256,"acquisition-contract.invalid-key");
  let url:URL;try{url=new URL(canonicalUrl);}catch{return fail("acquisition-contract.invalid-input");}
  if(url.protocol!=="https:"||url.origin!==expectedOrigin||url.username!==""||url.password!==""||url.hash!==""||(url.search!==""&&url.search!=="?redacted")||/%[0-9a-f]*[a-f][0-9a-f]*/u.test(url.pathname)||url.href!==canonicalUrl)fail("acquisition-contract.invalid-input");
...
function validateTraceSettlement(value:unknown,provider:AcquisitionProvider,operation:AcquisitionOperation,endpointClass:string):AcquisitionTraceSettlement {
  ...
  const expectedOrigin=expectedProviderOrigin(provider,endpointClass,operation);const requestedUrl=validateProjectedTraceUrl(v.requestedUrl,expectedOrigin);const finalUrl=v.finalUrl===null?null:validateProjectedTraceUrl(v.finalUrl,expectedOrigin);
  if(!Array.isArray(v.redirects))fail("acquisition-contract.invalid-input");let prior=requestedUrl.canonicalUrl;let ordinal=1;
  const redirects=v.redirects.map((item)=>{const r=requireClosed(item,["ordinal","status","from","to"],"acquisition-contract.invalid-input");if(r.ordinal!==ordinal||![301,302,303,307,308].includes(r.status as number))fail("acquisition-contract.invalid-input");ordinal+=1;const from=validateProjectedTraceUrl(r.from,expectedOrigin);const to=validateProjectedTraceUrl(r.to,expectedOrigin);if(from.canonicalUrl!==prior)fail("acquisition-contract.invalid-input");prior=to.canonicalUrl;return deepFreeze({ordinal:r.ordinal as number,status:r.status as 301|302|303|307|308,from,to});});
  if(finalUrl!==null&&finalUrl.canonicalUrl!==(redirects.length===0?requestedUrl.canonicalUrl:prior))fail("acquisition-contract.invalid-input");if(finalUrl===null&&redirects.length!==0)fail("acquisition-contract.invalid-input");
...
function computeCandidateGroups(candidates:readonly AcademicCandidate[]):readonly AcademicCandidateGroup[] {
  ...
  const strong=(candidate:AcademicCandidate):readonly string[]=>[candidate.identifiers.doi===null?null:`doi:${candidate.identifiers.doi}`,candidate.identifiers.pmid===null?null:`pmid:${candidate.identifiers.pmid}`,candidate.identifiers.pmcid===null?null:`pmcid:${candidate.identifiers.pmcid}`].filter((value):value is string=>value!==null);
  for(let left=0;left<candidates.length;left+=1)for(let right=left+1;right<candidates.length;right+=1){const leftStrong=strong(candidates[left]!);const rightStrong=new Set(strong(candidates[right]!));if(leftStrong.some((key)=>rightStrong.has(key))||(candidates[left]!.canonicalUrl!==null&&candidates[left]!.canonicalUrl===candidates[right]!.canonicalUrl))union(left,right);}
  ...
  for(const values of components.values()){
    const candidateKeys=values.map(({candidateKey})=>candidateKey).sort();const sets={doi:new Set<string>(),pmid:new Set<string>(),pmcid:new Set<string>()};for(const value of values)for(const kind of ["doi","pmid","pmcid"] as const){const identifier=value.identifiers[kind];if(identifier!==null)sets[kind].add(identifier);}
    const conflicting=(["doi","pmid","pmcid"] as const).some((kind)=>sets[kind].size>1);const sameUrl=values[0]!.canonicalUrl!==null&&values.every((value)=>value.canonicalUrl===values[0]!.canonicalUrl);
    const common=(["doi","pmid","pmcid"] as const).find((kind)=>{const first=values[0]!.identifiers[kind];return first!==null&&values.every((value)=>value.identifiers[kind]===first);});
    ...
    else if(common!==undefined){identityKind=common;identityValue=values[0]!.identifiers[common]!;status=conflicting?"ambiguous":"compatible";}
    else if(sameUrl){identityKind="canonical-url";identityValue=values[0]!.canonicalUrl!;status=conflicting?"ambiguous":"compatible";}
    else if(values.length===1){const first=(["doi","pmid","pmcid"] as const).find((kind)=>sets[kind].size===1);if(first!==undefined){identityKind=first;identityValue=[...sets[first]][0]!;}else if(values[0]!.canonicalUrl!==null){identityKind="canonical-url";identityValue=values[0]!.canonicalUrl;}else{identityKind="provider-result";identityValue=values[0]!.candidateKey;}status="compatible";}
    else {const keys=(["doi","pmid","pmcid"] as const).flatMap((kind)=>[...sets[kind]].map((value)=>({kind,value}))).sort((left,right)=>rank[left.kind]-rank[right.kind]||(left.value<right.value?-1:left.value>right.value?1:0));identityKind=keys[0]!.kind;identityValue=keys[0]!.value;status="ambiguous";}
  ...
  groups.sort((left,right)=>left.groupKey<right.groupKey?-1:left.groupKey>right.groupKey?1:0);return deepFreeze(groups);
}

...
type PinState={readonly runtime:PinnedHopRuntimeInternal;readonly deadline:RequestDeadlineInternal;readonly signal:AbortSignal;listener:(()=>void)|null;state:"resolved"|"consumed"|"stale"};
...
type PinnedRuntimeState={readonly node:NodeRuntimeCapabilitiesInternal;readonly ops:NodeOperationsInternal;readonly options:NetworkPolicyOptions|undefined;readonly dns:AcquisitionDnsResolver;open:boolean;closePromise:Promise<void>|null;readonly pins:Set<PinnedProviderTargetInternal>;readonly handles:Set<NodePinnedHopHandleInternal>;readonly owners:Set<string>};
...
function detachPinDeadlineListener(pin:PinState):void{const listener=pin.listener;if(listener===null)return;pin.listener=null;try{REMOVE_EVENT.call(pin.signal,"abort",listener);}catch{/* quarantine */}}
function staleResolvedPin(state:PinnedRuntimeState,target:PinnedProviderTargetInternal,pin:PinState):void{if(pin.state!=="resolved")return;pin.state="stale";state.pins.delete(target);detachPinDeadlineListener(pin);}
...
export function createPinnedHopRuntimeInternal(options:NetworkPolicyOptions|undefined,runtime:NodeRuntimeCapabilitiesInternal=createNodeRuntimeCapabilitiesInternal()):PinnedHopRuntimeInternal{
  const normalizedOptions=pinnedNetworkOptions(options),node=runtimeState(runtime),dns=createNodeDnsResolver(runtime);let output!:PinnedHopRuntimeInternal;
  const state:PinnedRuntimeState={node:runtime,ops:node.ops,options:normalizedOptions,dns,open:true,closePromise:null,pins:new Set(),handles:new Set(),owners:new Set()};
  output=Object.freeze({
    capabilityKind:"pinned-hop-runtime" as const,
    async resolveProviderTarget(url:unknown,deadline:RequestDeadlineInternal){
      if(!state.open)pinnedFail("pinned-runtime.closed");assertRequestDeadlineInternal(deadline);deadline.throwIfExpired();const validated=inferValidatedUrl(url);let safe;
      try{safe=await resolveAllSafeProviderAddressesInternal(validated,state.options,state.dns,deadline.signal);}catch(error){if(error instanceof NetworkPolicyError&&error.code==="network.cancelled")deadline.throwIfExpired();throw error;}
      deadline.throwIfExpired();if(!state.open)pinnedFail("pinned-runtime.closed");const first=safe.addresses[0]!,target=Object.freeze({capabilityKind:"pinned-provider-target" as const,...validated,address:first.address,family:first.family,allAddressesSha256:safe.allAddressesSha256});
      const pin:PinState={runtime:output,deadline,signal:deadline.signal,listener:null,state:"resolved"};const listener=()=>staleResolvedPin(state,target,pin);pin.listener=listener;pinStates.set(target,pin);state.pins.add(target);
      try{ADD_EVENT.call(pin.signal,"abort",listener,{once:true});}catch{staleResolvedPin(state,target,pin);return pinnedFail("pinned-runtime.invalid-capability");}
      if(ABORTED_GETTER.call(pin.signal))listener();if(pin.state!=="resolved")deadline.throwIfExpired();return target;
    },
    openPinnedHop(target:PinnedProviderTargetInternal,requestValue:PinnedHopOpenRequestInternal,callbacksValue:NodePinnedHopCallbacksInternal,deadline:RequestDeadlineInternal){
      const pin=target!==null&&typeof target==="object"&&!utilTypes.isProxy(target)?pinStates.get(target):undefined;if(!state.open){if(pin?.runtime===output&&pin.state==="stale")pinnedFail("pinned-runtime.pin-stale");pinnedFail("pinned-runtime.closed");}if(!pin||pin.runtime!==output)pinnedFail("pinned-runtime.invalid-capability");if(pin.state==="stale")pinnedFail("pinned-runtime.pin-stale");if(pin.state!=="resolved")pinnedFail("pinned-runtime.pin-replayed");if(pin.deadline!==deadline)pinnedFail("pinned-runtime.target-mismatch");
      const request=requestInput(requestValue),upper=callbacksInput(callbacksValue);if(request.url!==target.url)pinnedFail("pinned-runtime.target-mismatch");if(state.owners.has(request.ownerId))pinnedFail("pinned-runtime.pin-replayed");pin.state="consumed";state.pins.delete(target);detachPinDeadlineListener(pin);state.owners.add(request.ownerId);
      let handle:NodePinnedHopHandleInternal;try{handle=openHop(request.ownerId,target,request,upper,deadline,node);}catch(error){state.owners.delete(request.ownerId);throw error;}state.handles.add(handle);let cleaned=false;const cleanup=()=>{if(cleaned)return;cleaned=true;state.handles.delete(handle);state.owners.delete(request.ownerId);};
      try{void PROMISE_THEN.call(handle.completion,cleanup,cleanup);}catch{cleanup();try{handle.forceClose();}catch{/* quarantine */}return pinnedFail("pinned-runtime.invalid-capability");}return handle;
    },
    close(){if(state.closePromise!==null)return state.closePromise;state.open=false;let resolveClose!:()=>void,rejectClose!:(error:unknown)=>void;const completion=new Promise<void>((resolve,reject)=>{resolveClose=resolve;rejectClose=reject;});state.closePromise=completion;const task=(async()=>{for(const target of [...state.pins]){const pin=pinStates.get(target);if(pin?.runtime===output)staleResolvedPin(state,target,pin);}state.pins.clear();const dnsState=dnsStates.get(dns);if(dnsState)closeDnsState(dnsState);const active=[...state.handles];for(const handle of active)try{handle.abort();}catch{/* quarantine */}await Promise.all(active.map(async(handle)=>{try{await handle.close();}catch{/* quarantine */}}));state.handles.clear();state.owners.clear();})();try{void PROMISE_THEN.call(task,resolveClose,rejectClose);}catch{return pinnedFail("pinned-runtime.invalid-capability");}return completion;
    },
  });pinnedRuntimeStates.set(output,state);return output;
}
export function discardPinnedProviderTargetInternal(runtime:PinnedHopRuntimeInternal,target:PinnedProviderTargetInternal):void{const state=pinnedRuntimeState(runtime);if(target===null||typeof target!=="object"||utilTypes.isProxy(target))pinnedFail("pinned-runtime.invalid-capability");const pin=pinStates.get(target);if(!pin||pin.runtime!==runtime)pinnedFail("pinned-runtime.invalid-capability");if(pin.state==="consumed")pinnedFail("pinned-runtime.pin-replayed");if(pin.state==="stale")return;staleResolvedPin(state,target,pin);}
export function getPinnedHopRuntimeRetentionCountsInternal(runtime:PinnedHopRuntimeInternal):Readonly<{pins:number;handles:number;owners:number}>{const state=pinnedRuntimeState(runtime);return Object.freeze({pins:state.pins.size,handles:state.handles.size,owners:state.owners.size});}

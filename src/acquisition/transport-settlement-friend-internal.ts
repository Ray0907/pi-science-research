...
const SHA=/^[a-f0-9]{64}$/u,KEY=/^partition-v1-[a-f0-9]{64}$/u;
...
export function publishTransportSettlementInternal<T extends TransportSettlement>(settlement:T):T{
  if(settlement===null||typeof settlement!=="object"||utilTypes.isProxy(settlement))invalid();
  const stack:object[]=[settlement],seen=new WeakSet<object>();
  while(stack.length){const current=stack.pop()!;if(utilTypes.isProxy(current)||!Object.isFrozen(current)||seen.has(current))invalid();seen.add(current);for(const descriptor of Object.values(Object.getOwnPropertyDescriptors(current))){if(!("value" in descriptor))invalid();const child=descriptor.value;if(child!==null&&typeof child==="object")stack.push(child as object);}}
  const descriptors=Object.getOwnPropertyDescriptors(settlement),partitionKey=dataDescriptor(descriptors,"partitionKey"),partitionKeySha256=dataDescriptor(descriptors,"partitionKeySha256"),provider=dataDescriptor(descriptors,"provider"),operation=dataDescriptor(descriptors,"operation"),endpointClass=dataDescriptor(descriptors,"endpointClass"),requestedUrl=dataDescriptor(descriptors,"requestedUrl");

...
import { ALL_CAPABILITY_FLAGS, Capability, createCapabilityAnalysis, isFullyErasedTypeOnlyExport, isFullyErasedTypeOnlyImport, moduleCapabilityKind, unwrapExpression } from "./test-capability-analysis.js";
...
function auditChildSource(source: string, relative: string): ChildAudit {
    ...
    const abstractRootWitness = (expression: ts.Expression, seen = new Set<ts.Node>(), depth = 0): boolean => { if (depth >= 32 || seen.has(expression)) return false; seen.add(expression); const value = analysis.value(expression); if ((value.flags & rootMask) === 0) return false; if (value.flags !== ALL_CAPABILITY_FLAGS) return true; let found = false; ts.forEachChild(expression, (child) => { if (!found && ts.isExpression(child) && abstractRootWitness(child, seen, depth + 1)) found = true; }); const unwrapped = unwrapExpression(expression); if (!found && ts.isIdentifier(unwrapped)) for (const operation of analysis.operations) { let defines = false; const inspect = (node: ts.Node): void => { if (ts.isIdentifier(node) && node.text === unwrapped.text) defines = true; else ts.forEachChild(node, inspect); }; inspect(operation.pattern); if (defines && abstractRootWitness(operation.source, seen, depth + 1)) { found = true; break; } } return found; };

...
function resolvesPrivateModule(relative:string,expression:ts.Expression,target:string):boolean|null{const value=staticString(expression);if(value===undefined)return null;const normalized=value.startsWith(".")?path.posix.normalize(path.posix.join(path.posix.dirname(relative),value)):path.posix.normalize(value);return[`${target}.js`,`${target}.ts`].some(candidate=>normalized===candidate||normalized.endsWith(`/${candidate}`));}
function privateModuleReferences(source:string,relative:string,target:string):readonly string[]{const file=parse(source,relative),findings:string[]=[];for(const statement of file.statements){if(ts.isImportDeclaration(statement)&&ts.isStringLiteral(statement.moduleSpecifier)&&resolvesPrivateModule(relative,statement.moduleSpecifier,target))findings.push("import");if(ts.isExportDeclaration(statement)&&statement.moduleSpecifier&&ts.isStringLiteral(statement.moduleSpecifier)&&resolvesPrivateModule(relative,statement.moduleSpecifier,target))findings.push("export");}const visit=(node:ts.Node):void=>{if(ts.isCallExpression(node)&&node.expression.kind===ts.SyntaxKind.ImportKeyword){const argument=node.arguments[0];if(!argument||resolvesPrivateModule(relative,argument,target)!==false)findings.push("dynamic");}ts.forEachChild(node,visit);};visit(file);return findingSet(findings);}
type BootstrapModuleAudit=Readonly<{errors:readonly string[];imports:readonly string[]}>;
function auditBootstrapModuleSource(source:string,relative:string):BootstrapModuleAudit{const file=parse(source,relative),errors:string[]=[],imports:string[]=[];for(const statement of file.statements){if(ts.isImportDeclaration(statement)&&ts.isStringLiteral(statement.moduleSpecifier)&&resolvesPrivateModule(relative,statement.moduleSpecifier,"src/workflow/bootstrap")){const bindings=statement.importClause?.namedBindings;if(relative!=="tests/workflow/bootstrap.test.ts"||statement.moduleSpecifier.text!=="../../src/workflow/bootstrap.js")errors.push(`${relative}: bootstrap import site`);if(!statement.importClause||statement.importClause.name||!bindings||!ts.isNamedImports(bindings))errors.push(`${relative}: bootstrap import shape`);else for(const item of bindings.elements){if(item.propertyName)errors.push(`${relative}: bootstrap import alias`);imports.push(`${relative}:${item.isTypeOnly?"type":"runtime"}:${item.name.text}`);}}if(ts.isExportDeclaration(statement)&&statement.moduleSpecifier&&ts.isStringLiteral(statement.moduleSpecifier)&&resolvesPrivateModule(relative,statement.moduleSpecifier,"src/workflow/bootstrap"))errors.push(`${relative}: bootstrap export denied`);}const visit=(node:ts.Node):void=>{if(ts.isCallExpression(node)&&node.expression.kind===ts.SyntaxKind.ImportKeyword){const argument=node.arguments[0],resolved=argument&&resolvesPrivateModule(relative,argument,"src/workflow/bootstrap");if(resolved!==false)errors.push(`${relative}: ${resolved===null?"unresolved ":""}bootstrap dynamic import denied`);}ts.forEachChild(node,visit);};visit(file);return{errors:findingSet(errors),imports:findingSet(imports)};}
...
describe("default-deny test network policy", () => {
    ...
    test("confines lock retention observation to the exact lifecycle test",()=>{const symbols=new Set(["createResearchRunLockRetentionObserverInternal","getResearchRunLockRetentionCountsInternal"]),sites:string[]=[];for(const filePath of [...enumerate(path.join(root,"tests")),...enumerate(path.join(root,"src")),...enumerate(path.join(root,"extensions"))]){const relative=path.relative(root,filePath).replaceAll("\\","/"),source=fs.readFileSync(filePath,"utf8"),file=ts.createSourceFile(relative,source,ts.ScriptTarget.Latest,true,ts.ScriptKind.TS);for(const statement of file.statements)if(ts.isImportDeclaration(statement)&&ts.isStringLiteral(statement.moduleSpecifier)&&statement.importClause?.namedBindings&&ts.isNamedImports(statement.importClause.namedBindings))for(const item of statement.importClause.namedBindings.elements){const name=(item.propertyName??item.name).text;if(symbols.has(name))sites.push(`${name}:${item.isTypeOnly?"type":"runtime"}:${relative}:${statement.moduleSpecifier.text}`);}}expect(sites.sort()).toEqual(["createResearchRunLockRetentionObserverInternal:runtime:tests/storage/run-lock.test.ts:../../src/storage/run-lock-internal.js","getResearchRunLockRetentionCountsInternal:runtime:tests/storage/run-lock.test.ts:../../src/storage/run-lock-internal.js"]);});
    test("confines planning artifact test-control exports to the exact artifact test file",()=>{const symbols=new Set(["createPlanningArtifactTestControlInternal","resumePlanningArtifactTestControlInternal","getPlanningArtifactOpenHandleCountInternal","waitForPlanningArtifactPauseInternal"]),sites:string[]=[];for(const filePath of [...enumerate(path.join(root,"tests")),...enumerate(path.join(root,"src")),...enumerate(path.join(root,"extensions"))]){const relative=path.relative(root,filePath).replaceAll("\\","/"),source=fs.readFileSync(filePath,"utf8"),file=ts.createSourceFile(relative,source,ts.ScriptTarget.Latest,true,ts.ScriptKind.TS);for(const statement of file.statements)if(ts.isImportDeclaration(statement)&&ts.isStringLiteral(statement.moduleSpecifier)&&statement.importClause?.namedBindings&&ts.isNamedImports(statement.importClause.namedBindings))for(const item of statement.importClause.namedBindings.elements){const name=(item.propertyName??item.name).text;if(!item.isTypeOnly&&symbols.has(name))sites.push(`${name}:${relative}:${statement.moduleSpecifier.text}`);}}expect(sites.sort()).toEqual(["createPlanningArtifactTestControlInternal:tests/storage/planning-artifact.test.ts:../../src/storage/planning-artifact-internal.js","getPlanningArtifactOpenHandleCountInternal:tests/storage/planning-artifact.test.ts:../../src/storage/planning-artifact-internal.js","resumePlanningArtifactTestControlInternal:tests/storage/planning-artifact.test.ts:../../src/storage/planning-artifact-internal.js","waitForPlanningArtifactPauseInternal:tests/storage/planning-artifact.test.ts:../../src/storage/planning-artifact-internal.js"].sort());});
    test("confines every bootstrap module import and the failure getter to the exact bootstrap test",()=>{const errors:string[]=[],imports:string[]=[];for(const filePath of [...enumerate(path.join(root,"tests")),...enumerate(path.join(root,"src")),...enumerate(path.join(root,"extensions"))]){const relative=path.relative(root,filePath).replaceAll("\\","/"),audit=auditBootstrapModuleSource(fs.readFileSync(filePath,"utf8"),relative);errors.push(...audit.errors);imports.push(...audit.imports);}expect(errors).toEqual([]);expect(imports.sort()).toEqual(["tests/workflow/bootstrap.test.ts:runtime:ResearchBootstrapError","tests/workflow/bootstrap.test.ts:runtime:bootstrapResearchRunInternal","tests/workflow/bootstrap.test.ts:runtime:createResearchBootstrapContextInternal","tests/workflow/bootstrap.test.ts:runtime:createResearchBootstrapTestHooksInternal","tests/workflow/bootstrap.test.ts:runtime:createResolvedResearchRoleSnapshotInternal","tests/workflow/bootstrap.test.ts:runtime:getResearchBootstrapFailureInternal","tests/workflow/bootstrap.test.ts:runtime:projectResearchTopicInternal","tests/workflow/bootstrap.test.ts:type:ResearchBootstrapOperationFaultInternal","tests/workflow/bootstrap.test.ts:type:ResearchBootstrapPhaseInternal"].sort());for(const malicious of ['import * as bootstrap from "../../src/workflow/bootstrap.js";void bootstrap.getResearchBootstrapFailureInternal;', 'export {getResearchBootstrapFailureInternal} from "../../src/workflow/bootstrap.js";', 'import {getResearchBootstrapFailureInternal as leak} from "../../src/workflow/bootstrap.js";void leak;', 'void import("../../src/workflow/bootstrap.js");', 'import {getResearchBootstrapFailureInternal} from "../../src/workflow/./bootstrap.js";void getResearchBootstrapFailureInternal;', 'void import("../../src/workflow/"+"bootstrap.js");'])expect(auditBootstrapModuleSource(malicious,"tests/workflow/escape.test.ts").errors.length,malicious).toBeGreaterThan(0);});
    test("confines planning acceptance test capabilities to the exact acceptance test",()=>{const symbols=new Set(["createPlanningAcceptanceTestHooksInternal","getPlanningAcceptanceFailureInternal"]),sites:string[]=[];for(const filePath of [...enumerate(path.join(root,"tests")),...enumerate(path.join(root,"src")),...enumerate(path.join(root,"extensions"))]){const relative=path.relative(root,filePath).replaceAll("\\","/"),file=ts.createSourceFile(relative,fs.readFileSync(filePath,"utf8"),ts.ScriptTarget.Latest,true,ts.ScriptKind.TS);for(const statement of file.statements)if(ts.isImportDeclaration(statement)&&ts.isStringLiteral(statement.moduleSpecifier)&&statement.importClause?.namedBindings&&ts.isNamedImports(statement.importClause.namedBindings))for(const item of statement.importClause.namedBindings.elements){const name=(item.propertyName??item.name).text;if(!item.isTypeOnly&&symbols.has(name))sites.push(`${name}:${relative}:${statement.moduleSpecifier.text}`);}}expect(sites.sort()).toEqual(["createPlanningAcceptanceTestHooksInternal:tests/workflow/accepted-plan.test.ts:../../src/workflow/planning-acceptance-internal.js","createPlanningAcceptanceTestHooksInternal:tests/workflow/planning-acceptance.test.ts:../../src/workflow/planning-acceptance-internal.js","getPlanningAcceptanceFailureInternal:tests/workflow/planning-acceptance.test.ts:../../src/workflow/planning-acceptance-internal.js"].sort());});
    test("confines accepted plan test hooks to the exact accepted-plan test",()=>{const symbol="createAcceptedPlanTestHooksInternal",sites:string[]=[];for(const filePath of [...enumerate(path.join(root,"tests")),...enumerate(path.join(root,"src")),...enumerate(path.join(root,"extensions"))]){const relative=path.relative(root,filePath).replaceAll("\\","/"),file=ts.createSourceFile(relative,fs.readFileSync(filePath,"utf8"),ts.ScriptTarget.Latest,true,ts.ScriptKind.TS);for(const statement of file.statements)if(ts.isImportDeclaration(statement)&&ts.isStringLiteral(statement.moduleSpecifier)&&statement.importClause?.namedBindings&&ts.isNamedImports(statement.importClause.namedBindings))for(const item of statement.importClause.namedBindings.elements){const name=(item.propertyName??item.name).text;if(!item.isTypeOnly&&name===symbol)sites.push(`${name}:${relative}:${statement.moduleSpecifier.text}`);}}expect(sites).toEqual(["createAcceptedPlanTestHooksInternal:tests/workflow/accepted-plan.test.ts:../../src/workflow/accepted-plan-internal.js"]);});
    test("confines the lock hook factory to the exact lock and bootstrap tests",()=>{const symbol="createResearchRunLockTestHooksInternal",sites:string[]=[];for(const filePath of [...enumerate(path.join(root,"tests")),...enumerate(path.join(root,"src")),...enumerate(path.join(root,"extensions"))]){const relative=path.relative(root,filePath).replaceAll("\\","/"),file=ts.createSourceFile(relative,fs.readFileSync(filePath,"utf8"),ts.ScriptTarget.Latest,true,ts.ScriptKind.TS);for(const statement of file.statements)if(ts.isImportDeclaration(statement)&&ts.isStringLiteral(statement.moduleSpecifier)&&statement.importClause?.namedBindings&&ts.isNamedImports(statement.importClause.namedBindings)&&statement.importClause.namedBindings.elements.some(item=>!item.isTypeOnly&&(item.propertyName??item.name).text===symbol))sites.push(`${relative}:${statement.moduleSpecifier.text}`);}expect(sites.sort()).toEqual(["tests/storage/run-lock.test.ts:../../src/storage/run-lock-internal.js","tests/workflow/bootstrap.test.ts:../../src/storage/run-lock-internal.js"]);});
    test("locks every run-lock import to the exact source tests dynamic lifecycle test and generated fixture",()=>{const staticSites:string[]=[],dynamicSites:string[]=[],violations:string[]=[];for(const filePath of [...enumerate(path.join(root,"tests")),...enumerate(path.join(root,"src")),...enumerate(path.join(root,"extensions"))]){const relative=path.relative(root,filePath).replaceAll("\\","/"),source=fs.readFileSync(filePath,"utf8"),file=ts.createSourceFile(relative,source,ts.ScriptTarget.Latest,true,ts.ScriptKind.TS);for(const statement of file.statements){if(ts.isImportDeclaration(statement)&&ts.isStringLiteral(statement.moduleSpecifier)&&resolvesPrivateModule(relative,statement.moduleSpecifier,"src/storage/run-lock-internal")){const expectedSpecifier=relative==="src/workflow/bootstrap.ts"?"../storage/run-lock-internal.js":["tests/storage/run-lock.test.ts","tests/storage/run-root.test.ts","tests/workflow/bootstrap.test.ts"].includes(relative)?"../../src/storage/run-lock-internal.js":null;if(statement.moduleSpecifier.text!==expectedSpecifier)violations.push(`${relative}: lock import specifier`);const bindings=statement.importClause?.namedBindings;expect(bindings&&ts.isNamedImports(bindings),relative).toBe(true);for(const item of (bindings as ts.NamedImports).elements)staticSites.push(`${relative}:${item.isTypeOnly?"type":"runtime"}:${(item.propertyName??item.name).text}`);}if(ts.isExportDeclaration(statement)&&statement.moduleSpecifier&&ts.isStringLiteral(statement.moduleSpecifier)&&resolvesPrivateModule(relative,statement.moduleSpecifier,"src/storage/run-lock-internal"))violations.push(`${relative}: lock export`);}const visit=(node:ts.Node):void=>{if(ts.isCallExpression(node)&&node.expression.kind===ts.SyntaxKind.ImportKeyword){const argument=node.arguments[0],resolved=argument&&resolvesPrivateModule(relative,argument,"src/storage/run-lock-internal");if(resolved===null)violations.push(`${relative}: unresolved lock dynamic specifier`);else if(resolved){if(relative==="tests/storage/run-lock.test.ts"&&argument&&ts.isStringLiteral(argument)&&argument.text==="../../src/storage/run-lock-internal.js")dynamicSites.push(`${relative}:${argument.text}`);else violations.push(`${relative}: lock dynamic specifier`);}}ts.forEachChild(node,visit);};visit(file);}expect(violations).toEqual([]);expect(staticSites.sort()).toEqual(["src/workflow/bootstrap.ts:runtime:ResearchRunLockError","src/workflow/bootstrap.ts:runtime:acquireResearchRunLockInternal","src/workflow/bootstrap.ts:runtime:isResearchRunLockAcquisitionCleanupUncertainInternal","src/workflow/bootstrap.ts:runtime:isResearchRunLockTestHooksInternal","src/workflow/bootstrap.ts:type:ResearchRunLockInternal","src/workflow/bootstrap.ts:type:ResearchRunLockTestHooksInternal","tests/storage/run-lock.test.ts:runtime:ResearchRunLockError","tests/storage/run-lock.test.ts:runtime:acquireResearchRunLockInternal","tests/storage/run-lock.test.ts:runtime:createResearchRunLockRetentionObserverInternal","tests/storage/run-lock.test.ts:runtime:createResearchRunLockTestHooksInternal","tests/storage/run-lock.test.ts:runtime:getResearchRunLockRetentionCountsInternal","tests/storage/run-lock.test.ts:type:ResearchRunLockPhaseInternal","tests/storage/run-root.test.ts:runtime:acquireResearchRunLockInternal","tests/workflow/bootstrap.test.ts:runtime:createResearchRunLockTestHooksInternal","tests/workflow/bootstrap.test.ts:type:ResearchRunLockPhaseInternal"].sort());expect(dynamicSites).toHaveLength(8);expect(new Set(dynamicSites)).toEqual(new Set(["tests/storage/run-lock.test.ts:../../src/storage/run-lock-internal.js"]));const generated=fs.readFileSync(path.join(root,"tests/storage/run-root.test.ts"),"utf8");expect(generated.match(/const lockSource = resolve\("src\/storage\/run-lock-internal\.ts"\);/gu)).toHaveLength(1);expect(generated.match(/import \{ acquireResearchRunLockInternal \} from \$\{JSON\.stringify\(lockSource\)\};/gu)).toHaveLength(1);for(const malicious of ['import {acquireResearchRunLockInternal} from "../../src/storage/./run-lock-internal.js";void acquireResearchRunLockInternal;', 'void import("../../src/storage/"+"run-lock-internal.js");'])expect(privateModuleReferences(malicious,"tests/storage/escape.test.ts","src/storage/run-lock-internal").length,malicious).toBeGreaterThan(0);});

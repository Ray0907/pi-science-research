...
export function createMockSecureTransportCapabilities():MockSecureTransportControl {
  let timerId=0;const timers=new Map<number,()=>void>();const requests:MockSecureTransportControl["requests"]=[];const agents:MockSecureTransportControl["agents"]=[];const resolvers:MockSecureTransportControl["resolvers"]=[];
  const control={} as MockSecureTransportControl;control.now=100;control.timers=timers;control.requests=requests;control.agents=agents;control.resolvers=resolvers;control.resolve4=async()=>["8.8.8.8"];control.resolve6=async()=>["2606:4700:4700::1111"];
  const ops:NodeOperationsInternal={
    ...
    clock:{monotonicNow:()=>control.now,timestampNow:()=>new Date(1_700_000_000_000+control.now).toISOString(),setTimer:(callback)=>{const id=++timerId;timers.set(id,callback);return id;},clearTimer:(handle)=>{timers.delete(handle as number);}},
  };

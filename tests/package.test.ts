...
describe("Pi package manifest", () => {
  ...
  it("keeps the package CI cardinality synchronized to the observed unique tarball",async()=>{const workflow=await readFile(new URL("../.github/workflows/ci.yml",import.meta.url),"utf8");expect([...workflow.matchAll(/paths\.length !== 58 \|\| new Set\(paths\)\.size !== 58/gu)]).toHaveLength(1);expect(workflow).not.toMatch(/paths\.length !== 57|size !== 57/gu);});
});

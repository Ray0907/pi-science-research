...
describe("Pi package manifest", () => {
  ...
  it("does not root-export TransportSettlement transport DNS provider result-registry getter view normalizeAcquisitionOptions normalized options planAcquisitionInvocation planning capability plan snapshots sink factory dependency raw payload socket header operation workflow or lock seams", async () => {
    ...
    const rootIndex=new URL("../src/index.ts",import.meta.url),confinedUrls=["../src/workflow/depth-profile.ts","../src/workflow/research-options.ts","../src/workflow/bootstrap.ts","../src/storage/run-lock-internal.ts","../src/storage/planning-artifact-internal.ts","../src/workflow/planning-acceptance-internal.ts","../src/workflow/accepted-plan-internal.ts"].map(relative=>new URL(relative,import.meta.url)),confinedPaths=new Set(confinedUrls.map(url=>url.pathname));
  ...
  it("publishes acquisition provider transport and extension runtime files only", async () => {
    ...
    expect(paths).toEqual(expect.arrayContaining([
      ...
      "src/workflow/planning-acceptance-internal.ts",
      "src/workflow/accepted-plan-internal.ts",
    ]));
  });
  ...
  it("keeps the package CI cardinality synchronized to the observed unique tarball",async()=>{const workflow=await readFile(new URL("../.github/workflows/ci.yml",import.meta.url),"utf8");expect([...workflow.matchAll(/paths\.length !== 59 \|\| new Set\(paths\)\.size !== 59/gu)]).toHaveLength(1);expect(workflow).not.toMatch(/paths\.length !== 58|size !== 58/gu);});
});

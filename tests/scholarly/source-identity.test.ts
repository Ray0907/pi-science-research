...
describe("source identity and provenance", () => {
  ...
  test("does not invoke comparison sort in merge or provenance build ordering", () => { const base = src("src-radix.000000001", { identifiers: { doi: DOI, pmid: null, pmcid: null }, retrievalRequestIds: [REQUEST_A] }); const update = src("src-radix.000000002", { identifiers: { doi: DOI, pmid: null, pmcid: null } }); const merged = withoutComparisonSort(() => mergeSourceRecords([base], [update])); expect(merged.sources).toHaveLength(1); const request = req(REQUEST_A, base.sourceId, { finalUrl: base.canonicalUrl }); const index = withoutComparisonSort(() => buildRequestProvenanceIndex([base], [request])); expect(validateSourceCanonicalUrlProvenance(base, index)).toMatchObject({ requestId: REQUEST_A }); });

  test("maps an internal configured radix overflow to the closed provenance code", () => { const base = src("src-bound.000000001", { identifiers: { doi: DOI, pmid: null, pmcid: null }, retrievalRequestIds: [REQUEST_A] }); const incoming = src("src-bound.000000002", { identifiers: { doi: DOI, pmid: null, pmcid: null }, retrievalRequestIds: [REQUEST_B] }); expect(errorCode(() => mergeSourceRecords([base], [incoming], { maxProvenanceSteps: 1 }))).toBe("source.too-many-provenance-steps"); });

  test("accepts the Task 4 configured provenance ceiling above the removed internal cap", () => { const index = buildRequestProvenanceIndexForEvidenceSnapshotInternal([], [], { maxProvenanceSteps: 2_000_000 }); expect(index).toMatchObject({ sourceCount: 0, requestCount: 0, witnessCount: 0 }); });

  test("orders shuffled and reversed 10k provenance witnesses byte-identically with linear diagnostics", () => { const requestIds = Array.from({ length: 10_000 }, (_, index) => `request-${String(index + 1).padStart(16, "0")}`); const record = src("src-witness.0000001", { canonicalUrl: "https://example.org/witness", retrievalRequestIds: [...requestIds].reverse() }); const requests = requestIds.map((id) => req(id, record.sourceId, { finalUrl: record.canonicalUrl })); const shuffled = requests.map((_value, index) => requests[(index * 7919) % requests.length]!); const limits = { maxSources: 1, maxRequests: 10_000, maxProvenanceSteps: 100_000, maxSourceRecordCanonicalBytes: 1_048_576, maxAggregateCanonicalBytes: 67_108_864 }; const firstDiagnostics = diagnostics(); const secondDiagnostics = diagnostics(); const first = buildRequestProvenanceIndex([record], [...requests].reverse(), limits, firstDiagnostics); const secondRecord = { ...record, retrievalRequestIds: requestIds }; const second = buildRequestProvenanceIndex([secondRecord], shuffled, limits, secondDiagnostics); const firstWitness = validateSourceCanonicalUrlProvenance(record, first); const secondWitness = validateSourceCanonicalUrlProvenance(secondRecord, second); expect(canonicalJson(firstWitness)).toBe(canonicalJson(secondWitness)); expect(firstDiagnostics).toEqual(secondDiagnostics); expect(firstDiagnostics.requestVisits).toBe(10_000); expect(firstDiagnostics.witnessInsertions).toBe(10_000); expect(firstDiagnostics.requestUrlVisits).toBeLessThanOrEqual(30_000); });

  test("orders merge and provenance values by JS code units across permutations", () => { const bmp = "\uE000"; const astral = "\u{10000}"; const author = (family: string) => ({ family, given: null, literal: null, orcid: null }); const provenance = [{ field: "lineage.datasetIds", provider: "openalex" as const, requestId: REQUEST_A }, { field: "lineage.cohortIds", provider: "openalex" as const, requestId: REQUEST_A }, { field: "authors", provider: "openalex" as const, requestId: REQUEST_A }]; const base = src("src-codeunit.0000001", { identifiers: { doi: DOI, pmid: null, pmcid: null }, authors: [author(bmp)], retrievalRequestIds: [REQUEST_A], metadataProvenance: provenance, lineage: { studyId: null, cohortIds: [bmp], datasetIds: [bmp], relatedSourceIds: [], relationTypes: [] } }); const incoming = src("src-codeunit.0000002", { identifiers: { doi: DOI, pmid: null, pmcid: null }, authors: [author(astral)], retrievalRequestIds: [REQUEST_A], metadataProvenance: [...provenance].reverse(), lineage: { studyId: null, cohortIds: [astral], datasetIds: [astral], relatedSourceIds: [], relationTypes: [] } }); const first = mergeSourceRecords([base], [incoming]).sources.at(-1)!; const second = mergeSourceRecords([{ ...base, authors: [...base.authors].reverse(), metadataProvenance: [...base.metadataProvenance].reverse() }], [{ ...incoming, authors: [...incoming.authors].reverse() }]).sources.at(-1)!; for (const merged of [first, second]) { expect(merged.authors.map(({ family }) => family)).toEqual([astral, bmp]); expect(merged.lineage.cohortIds).toEqual([astral, bmp]); expect(merged.lineage.datasetIds).toEqual([astral, bmp]); expect(merged.metadataProvenance.map(({ field }) => field)).toEqual(["authors", "lineage.cohortIds", "lineage.datasetIds"]); } expect(canonicalJson(first)).toBe(canonicalJson(second)); });

  test("returns fresh deeply frozen public provenance witnesses without cache aliases", () => { const source = src("src-fresh.000000001", { canonicalUrl: "https://fresh.example.org/a", retrievalRequestIds: [REQUEST_A] }); const request = req(REQUEST_A, source.sourceId, { finalUrl: source.canonicalUrl }); const index = buildRequestProvenanceIndex([source], [request]); const optionsHash = index.optionsSha256; const policyHash = index.policySha256; const first = validateSourceCanonicalUrlProvenance(source, index); const second = validateSourceCanonicalUrlProvenance(source, index); expect(first).not.toBe(second); expect(canonicalJson(first)).toBe(canonicalJson(second)); expect(Object.isFrozen(first)).toBe(true); expect(() => { (first as { kind: string }).kind = "changed"; }).toThrow(); const issued = validatedProvenanceRecordsForSnapshot(index).sources[0]!; const internalBefore = canonicalJson(validateSourceCanonicalUrlProvenanceFromSnapshotInternal(issued, index)); const third = validateSourceCanonicalUrlProvenance(source, index); expect(third).not.toBe(first); expect(canonicalJson(validateSourceCanonicalUrlProvenanceFromSnapshotInternal(issued, index))).toBe(internalBefore); expect(index.optionsSha256).toBe(optionsHash); expect(index.policySha256).toBe(policyHash); });

  test("reports exact closed SourceIdentityError codes", () => { const approved = ["source.invalid-options", "source.invalid-input", "source.too-many-records", "source.too-many-provenance-steps", "source.record-too-large", "source.input-too-large", "source.duplicate-revision", "source.revision-gap", "source.invalid-provenance", "source.invalid-lineage", "source.url-policy-invalid", "source.url-unattributed", "source.url-request-mismatch", "source.url-metadata-mismatch", "source.provenance-index-mismatch"] as const satisfies readonly SourceIdentityErrorCode[]; const exhaustive: [Exclude<SourceIdentityErrorCode, (typeof approved)[number]>] extends [never] ? true : false = true; expect(exhaustive).toBe(true); expect(errorCode(() => mergeSourceRecords([src("src-semantic.000001", { identifiers: { doi: "not-a-doi", pmid: null, pmcid: null } })], []))).toBe("source.invalid-input");

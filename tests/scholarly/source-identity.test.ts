...
describe("source identity and provenance", () => {
  ...
  test("bounds sources and provenance before grouping", () => {
    ...
    expect(errorCode(() => validateProspectiveSourceSemantics(crowded, { maxProvenanceSteps: 1 }))).toBe("source.too-many-provenance-steps");
  ...
  test("chooses one canonical provenance witness independent of request redirect and metadata order", () => {
    const source = src("src-ordering.0000001", { canonicalUrl: "https://article.example.org/a", retrievalRequestIds: [REQUEST_B, REQUEST_A] });
    ...
    for (const requests of [
      ...
      expect(validateSourceCanonicalUrlProvenanceOnce(source, requests)).toEqual({ kind: "transport-url", requestId: REQUEST_A, matchedField: "requestedUrl" });
    }
  });
  ...
  test("orders URL failures as unsafe invalid-status-or-link provider then missing independent of input order", () => {
    ...
    expect(errorCode(() => validateSourceCanonicalUrlProvenanceOnce(unsafe, [req(REQUEST_A, unsafe.sourceId, { status: "terminal-error" })]))).toBe("source.url-policy-invalid");
    const source = { ...unsafe, canonicalUrl: "https://article.example.org/a" };
    expect(errorCode(() => validateSourceCanonicalUrlProvenanceOnce(source, [req(REQUEST_A, "src-other.000000001", { status: "terminal-error" })]))).toBe("source.url-request-mismatch");
    const provider = { ...source, retrievalRequestIds: [REQUEST_A], metadataProvenance: [{ field: "canonicalUrl", provider: "crossref", requestId: REQUEST_A }] };
    expect(errorCode(() => validateSourceCanonicalUrlProvenanceOnce(provider, [req(REQUEST_A, provider.sourceId)]))).toBe("source.url-metadata-mismatch");
    expect(errorCode(() => validateSourceCanonicalUrlProvenanceOnce({ ...source, retrievalRequestIds: [] }, []))).toBe("source.url-unattributed");
  });
  ...
  test("bounds canonical source request and aggregate bytes before grouping", () => {
    ...
    expect(validateProspectiveSourceSemantics(source, { maxSourceRecordCanonicalBytes: bytes })).toEqual(source);
    expect(errorCode(() => validateProspectiveSourceSemantics(source, { maxSourceRecordCanonicalBytes: bytes - 1 }))).toBe("source.record-too-large");
    ...
    expect(buildRequestProvenanceIndex([source], [request], { maxRequestRecordCanonicalBytes: requestBytes })).toBeDefined();
    expect(errorCode(() => buildRequestProvenanceIndex([source], [request], { maxRequestRecordCanonicalBytes: requestBytes - 1 }))).toBe("source.record-too-large");
    const aggregateBytes = Buffer.byteLength(canonicalJson({ existing: [], incoming: [source] }));
    expect(errorCode(() => mergeSourceRecords([], [source], { maxAggregateCanonicalBytes: aggregateBytes - 1 }))).toBe("source.input-too-large");
  });
  ...
  test("rejects oversized title metadata request query and nested strings", () => {
    ...
    expect(errorCode(() => validateProspectiveSourceSemantics(source, { maxSourceRecordCanonicalBytes: 1_000 }))).toBe("source.record-too-large");
    ...
    expect(errorCode(() => buildRequestProvenanceIndex([src("src-request.0000001")], [request], { maxRequestRecordCanonicalBytes: 1_000 }))).toBe("source.record-too-large");
  });

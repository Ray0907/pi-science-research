...
function queryDiagnostics(): EvidenceQueryDiagnostics { return { indexVisits: 0, candidateVisits: 0, closureVisits: 0, normalizedQueryCanonicalizations: 0, selectedWrapperCanonicalizations: 0 }; }
function queryCorpus(count: number): CanonicalEvidenceSet { const claims = Array.from({ length: count }, (_, n) => claim(`claim-${String(n + 1).padStart(16, "0")}`, { evidenceRefs: [{ evidenceId: `ev-${String(n + 1).padStart(16, "0")}`, revision: 1 }] })); const evidenceValues = claims.map((item, n) => evidence(`ev-${String(n + 1).padStart(16, "0")}`, { claimRef: { claimId: item.claimId, revision: 1 } })); return records({ claims, evidence: evidenceValues }); }
function selectedWrapper(value: ReturnType<ReturnType<typeof buildEvidenceIndex>["query"]>): string { return canonicalJson({ calculations: value.calculations, claims: value.claims, evidence: value.evidence, requests: value.requests, sources: value.sources, verifications: value.verifications }); }
...
describe("bounded evidence queries", () => {
  ...
  test("canonicalizes one selected wrapper for a 1,000-primary bounded closure page", () => { const s = snapshot(queryCorpus(1_001)); const baseline = buildEvidenceIndex({ snapshot: s }).query(query({ limit: 1_000, maxSelectedBytes: 8_000_000 })); const d = queryDiagnostics(); const actual = buildEvidenceIndexWithDiagnosticsInternal({ snapshot: s }, undefined, d).query(query({ limit: 1_000, maxSelectedBytes: 8_000_000 })); expect(actual.evidence).toHaveLength(1_000); expect(actual.claims).toHaveLength(1_000); expect(actual.selectedRecordBundleSha256).toBe(sha256Hex(selectedWrapper(actual))); expect(canonicalJson(actual)).toBe(canonicalJson(baseline)); expect(actual.nextCursor).toBe(baseline.nextCursor); expect(d.selectedWrapperCanonicalizations).toBe(1); expect(d.candidateVisits).toBe(1_001); expect(d.closureVisits).toBe(2_001); });
  test("keeps the empty six-array selection wrapper exact", () => { const d = queryDiagnostics(); const empty = buildEvidenceIndexWithDiagnosticsInternal({ snapshot: snapshot() }, undefined, d).query(query({ stances: ["neutral"] })); expect(empty.sources).toEqual([]); expect(empty.claims).toEqual([]); expect(empty.evidence).toEqual([]); expect(empty.selectedRecordBundleSha256).toBe(sha256Hex(selectedWrapper(empty))); expect(Buffer.byteLength(selectedWrapper(empty), "utf8")).toBe(Buffer.byteLength('{"calculations":[],"claims":[],"evidence":[],"requests":[],"sources":[],"verifications":[]}', "utf8")); expect(d.selectedWrapperCanonicalizations).toBe(1); });
  test("canonicalizes the final wrapper once after truncation rollback and enforces exact bytes", () => { const s = snapshot(queryCorpus(2)); const per = { sources: 1, claims: 1, evidence: 2, verifications: 1, requests: 1, calculations: 1 }; const options = { maxSelectedPerKind: per, maxTotalSelectedRecords: 4 }; const d = queryDiagnostics(); const truncated = buildEvidenceIndexWithDiagnosticsInternal({ snapshot: s }, options, d).query(query({ limit: 2 })); expect(truncated.truncated).toBe(true); expect(truncated.evidence).toHaveLength(1); expect(truncated.claims).toHaveLength(1); expect(d.selectedWrapperCanonicalizations).toBe(1); expect(d.closureVisits).toBe(5); const exactBytes = Buffer.byteLength(selectedWrapper(truncated), "utf8"); const exactDiagnostics = queryDiagnostics(); const exact = buildEvidenceIndexWithDiagnosticsInternal({ snapshot: s }, options, exactDiagnostics).query(query({ limit: 2, maxSelectedBytes: exactBytes })); expect(exact.selectedRecordBundleSha256).toBe(truncated.selectedRecordBundleSha256); expect(exactDiagnostics.selectedWrapperCanonicalizations).toBe(1); expect(errorCode(() => buildEvidenceIndex({ snapshot: s }, options).query(query({ evidenceRefs: [{ evidenceId: s.records.evidence[0]!.evidenceId, revision: 1 }], maxSelectedBytes: exactBytes - 1 })))).toBe("query.closure-too-large"); });
